...
import wang.yeting.sql.dialect.ads.visitor.AdsOutputVisitor;
import wang.yeting.sql.dialect.antspark.visitor.AntsparkOutputVisitor;
import wang.yeting.sql.dialect.blink.vsitor.BlinkOutputVisitor;
import wang.yeting.sql.dialect.clickhouse.visitor.ClickhouseOutputVisitor;
import wang.yeting.sql.dialect.db2.visitor.DB2OutputVisitor;
import wang.yeting.sql.dialect.h2.visitor.H2OutputVisitor;
import wang.yeting.sql.dialect.hive.visitor.HiveOutputVisitor;
import wang.yeting.sql.dialect.mysql.ast.MySqlObject;
import wang.yeting.sql.dialect.mysql.ast.clause.MySqlSelectIntoStatement;
import wang.yeting.sql.dialect.mysql.ast.statement.MySqlInsertStatement;
import wang.yeting.sql.dialect.mysql.visitor.MySqlOutputVisitor;
import wang.yeting.sql.dialect.odps.visitor.OdpsOutputVisitor;
import wang.yeting.sql.dialect.oracle.visitor.OracleOutputVisitor;
import wang.yeting.sql.dialect.postgresql.visitor.PGOutputVisitor;
import wang.yeting.sql.dialect.sqlserver.visitor.SQLServerOutputVisitor;
import wang.yeting.sql.logging.Log;
import wang.yeting.sql.logging.LogFactory;
import wang.yeting.sql.parser.*;
import wang.yeting.sql.util.*;
...
public class SQLUtils {
    ...
    public static String toSQLString(wang.yeting.sql.ast.SQLObject sqlObject, String dbType) {
        return toSQLString(sqlObject, DbType.valueOf(dbType));
    }

    public static String toSQLString(wang.yeting.sql.ast.SQLObject sqlObject, DbType dbType) {
        return toSQLString(sqlObject, dbType, null, null);
    }

    public static String toSQLString(wang.yeting.sql.ast.SQLObject sqlObject, DbType dbType, FormatOption option) {
        return toSQLString(sqlObject, dbType, option, null);
    }

    public static String toSQLString(wang.yeting.sql.ast.SQLObject sqlObject, DbType dbType, FormatOption option, VisitorFeature... features) {
    ...
    public static String toSQLString(wang.yeting.sql.ast.SQLObject obj) {
    ...
    public static String toOdpsString(wang.yeting.sql.ast.SQLObject sqlObject) {
        return toOdpsString(sqlObject, null);
    }

    public static String toHiveString(wang.yeting.sql.ast.SQLObject sqlObject) {
        return toSQLString(sqlObject, DbType.odps);
    }

    public static String toOdpsString(wang.yeting.sql.ast.SQLObject sqlObject, FormatOption option) {
        return toSQLString(sqlObject, DbType.odps, option);
    }

    public static String toAntsparkString(wang.yeting.sql.ast.SQLObject sqlObject) {
        return toAntsparkString(sqlObject, null);
    }

    public static String toAntsparkString(wang.yeting.sql.ast.SQLObject sqlObject, FormatOption option) {
        return toSQLString(sqlObject, DbType.antspark, option);
    }

    public static String toMySqlString(wang.yeting.sql.ast.SQLObject sqlObject) {
        return toMySqlString(sqlObject, (FormatOption) null);
    }

    public static String toMySqlStringIfNotNull(wang.yeting.sql.ast.SQLObject sqlObject, String defaultStr) {
        if (sqlObject == null) {
            return defaultStr;
        }
    ...
    public static String toMySqlString(wang.yeting.sql.ast.SQLObject sqlObject, VisitorFeature... features) {
        return toMySqlString(sqlObject, new FormatOption(features));
    }

    public static String toNormalizeMysqlString(wang.yeting.sql.ast.SQLObject sqlObject) {
        if (sqlObject != null) {
    ...
    public static String toMySqlString(wang.yeting.sql.ast.SQLObject sqlObject, FormatOption option) {
        return toSQLString(sqlObject, DbType.mysql, option);
    }

    public static wang.yeting.sql.ast.SQLExpr toMySqlExpr(String sql) {
    ...
    public static String toOracleString(wang.yeting.sql.ast.SQLObject sqlObject) {
        return toOracleString(sqlObject, null);
    }

    public static String toOracleString(wang.yeting.sql.ast.SQLObject sqlObject, FormatOption option) {
        return toSQLString(sqlObject, DbType.oracle, option);
    }

    public static String toPGString(wang.yeting.sql.ast.SQLObject sqlObject) {
        return toPGString(sqlObject, null);
    }

    public static String toPGString(wang.yeting.sql.ast.SQLObject sqlObject, FormatOption option) {
        return toSQLString(sqlObject, DbType.postgresql, option);
    }

    public static String toDB2String(wang.yeting.sql.ast.SQLObject sqlObject) {
        return toDB2String(sqlObject, null);
    }

    public static String toDB2String(wang.yeting.sql.ast.SQLObject sqlObject, FormatOption option) {
        return toSQLString(sqlObject, DbType.db2, option);
    }

    public static String toSQLServerString(wang.yeting.sql.ast.SQLObject sqlObject) {
        return toSQLServerString(sqlObject, null);
    }

    public static String toSQLServerString(wang.yeting.sql.ast.SQLObject sqlObject, FormatOption option) {
    ...
    public static wang.yeting.sql.ast.SQLExpr toSQLExpr(String sql, DbType dbType) {
        SQLExprParser parser = SQLParserUtils.createExprParser(sql, dbType);
        wang.yeting.sql.ast.SQLExpr expr = parser.expr();
    ...
    public static wang.yeting.sql.ast.SQLExpr toSQLExpr(String sql) {
...
//            }
    ...
    public static SQLStatement parseSingleStatement(String sql, String dbType) throws RuntimeException {
        return parseSingleStatement(sql, DbType.of(dbType));
    }

    public static SQLStatement parseSingleStatement(String sql, DbType dbType) throws RuntimeException {
        return parseSingleStatement(sql, dbType, false);
    }
    ...
    public static List<wang.yeting.sql.ast.SQLExpr> split(SQLBinaryOpExpr x) {
    ...
    public static String addCondition(String sql, String condition, SQLBinaryOperator op, boolean left, DbType dbType) {
        ...
        wang.yeting.sql.ast.SQLExpr conditionExpr = toSQLExpr(condition, dbType);
    ...
    public static void addCondition(SQLStatement stmt, SQLBinaryOperator op, wang.yeting.sql.ast.SQLExpr condition, boolean left) {
        if (stmt instanceof wang.yeting.sql.ast.statement.SQLSelectStatement) {
            SQLSelectQuery query = ((wang.yeting.sql.ast.statement.SQLSelectStatement) stmt).getSelect().getQuery();
            if (query instanceof SQLSelectQueryBlock) {
                SQLSelectQueryBlock queryBlock = (SQLSelectQueryBlock) query;
                wang.yeting.sql.ast.SQLExpr newCondition = buildCondition(op, condition, left, queryBlock.getWhere());
        ...
        if (stmt instanceof SQLDeleteStatement) {
            ...
            wang.yeting.sql.ast.SQLExpr newCondition = buildCondition(op, condition, left, delete.getWhere());
        ...
        if (stmt instanceof SQLUpdateStatement) {
            ...
            wang.yeting.sql.ast.SQLExpr newCondition = buildCondition(op, condition, left, update.getWhere());
    ...
    public static wang.yeting.sql.ast.SQLExpr buildCondition(SQLBinaryOperator op, wang.yeting.sql.ast.SQLExpr condition, boolean left, wang.yeting.sql.ast.SQLExpr where) {
        if (where == null) {
            return condition;
        }
    ...
    public static String addSelectItem(String selectSql, String expr, String alias, boolean first, DbType dbType) {
        ...
        wang.yeting.sql.ast.SQLExpr columnExpr = toSQLExpr(expr, dbType);
    ...
    public static void addSelectItem(SQLStatement stmt, wang.yeting.sql.ast.SQLExpr expr, String alias, boolean first) {
        if (expr == null) {
            return;
        }
    ...
    public static void addSelectItem(SQLSelectQueryBlock queryBlock, wang.yeting.sql.ast.SQLExpr expr, String alias, boolean first) {
    ...
    public static wang.yeting.sql.ast.SQLExpr not(wang.yeting.sql.ast.SQLExpr expr) {
    ...
    public static boolean nameEquals(wang.yeting.sql.ast.SQLName a, wang.yeting.sql.ast.SQLName b) {
        if (a == b) {
            return true;
        }
    ...
    public static boolean isValue(wang.yeting.sql.ast.SQLExpr expr) {
    ...
    public static boolean replaceInParent(wang.yeting.sql.ast.SQLExpr expr, wang.yeting.sql.ast.SQLExpr target) {
        if (expr == null) {
            return false;
        }
    ...
    public static boolean replaceInParent(SQLTableSource cmp, SQLTableSource dest) {
        ...
        wang.yeting.sql.ast.SQLObject parent = cmp.getParent();
    ...
    public static Object[] clearLimit(String query, DbType dbType) {
        ...
        wang.yeting.sql.ast.SQLLimit limit = null;

        SQLStatement statement = (SQLStatement) stmtList.get(0);

        if (statement instanceof wang.yeting.sql.ast.statement.SQLSelectStatement) {
            wang.yeting.sql.ast.statement.SQLSelectStatement selectStatement = (wang.yeting.sql.ast.statement.SQLSelectStatement) statement;
    ...
    private static wang.yeting.sql.ast.SQLLimit clearLimit(SQLSelectQueryBlock queryBlock) {
        if (queryBlock == null) {
            return null;
        }

        wang.yeting.sql.ast.SQLLimit limit = queryBlock.getLimit();
        queryBlock.setLimit(null);
        return limit;
    }

    public static wang.yeting.sql.ast.SQLLimit getLimit(SQLStatement statement, DbType dbType) {
        if (statement instanceof wang.yeting.sql.ast.statement.SQLSelectStatement) {
            SQLSelectQueryBlock queryBlock = ((wang.yeting.sql.ast.statement.SQLSelectStatement) statement).getSelect().getQueryBlock();
    ...
    public static wang.yeting.sql.ast.SQLLimit getLimit(String query, DbType dbType) {
        List stmtList = SQLUtils.parseStatements(query, dbType);
        SQLStatement statement = (SQLStatement) stmtList.get(0);
