 ...
 */
package wang.yeting.sql.logging;
...
public class LogFactory {
    ...
    static {
        ...
        if (logType != null) {
            if (logType.equalsIgnoreCase("slf4j")) {
                tryImplementation("org.slf4j.Logger", "wang.yeting.sql.logging.SLF4JImpl");
            } else if (logType.equalsIgnoreCase("log4j")) {
                tryImplementation("org.apache.log4j.Logger", "wang.yeting.sql.logging.Log4jImpl");
            } else if (logType.equalsIgnoreCase("log4j2")) {
                tryImplementation("org.apache.logging.log4j.Logger", "wang.yeting.sql.logging.Log4j2Impl");
            } else if (logType.equalsIgnoreCase("commonsLog")) {
                tryImplementation("org.apache.commons.logging.LogFactory",
                        "wang.yeting.sql.logging.JakartaCommonsLoggingImpl");
            } else if (logType.equalsIgnoreCase("jdkLog")) {
                tryImplementation("java.util.logging.Logger", "wang.yeting.sql.logging.Jdk14LoggingImpl");
            }
        }
    ...
    public static synchronized void selectLog4JLogging() {
        try {
            ...
            Class implClass = Resources.classForName("wang.yeting.sql.logging.Log4jImpl");
            logConstructor = implClass.getConstructor(new Class[]{String.class});
        } catch (Throwable t) {
            //ignore
        }
    ...
    public static synchronized void selectJavaLogging() {
        try {
            ...
            Class implClass = Resources.classForName("wang.yeting.sql.logging.Jdk14LoggingImpl");
            logConstructor = implClass.getConstructor(new Class[]{String.class});
        } catch (Throwable t) {
            //ignore
        }
