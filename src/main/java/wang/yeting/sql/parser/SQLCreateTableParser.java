...
import wang.yeting.sql.ast.SQLPartitionBy;
import wang.yeting.sql.dialect.oracle.parser.OracleSelectParser;
...
public class SQLCreateTableParser extends SQLDDLParser {
    ...
    public wang.yeting.sql.ast.statement.SQLCreateTableStatement parseCreateTable() {
        ...
        wang.yeting.sql.ast.statement.SQLCreateTableStatement stmt = parseCreateTable(true);
        if (comments != null) {
    ...
    public wang.yeting.sql.ast.statement.SQLCreateTableStatement parseCreateTable(boolean acceptCreate) {
        wang.yeting.sql.ast.statement.SQLCreateTableStatement createTable = newCreateStatement();
        ...
        if (lexer.identifierEquals("GLOBAL")) {
            ...
            if (lexer.identifierEquals("TEMPORARY")) {
                lexer.nextToken();
                createTable.setType(wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type.GLOBAL_TEMPORARY);
            } else {
            ...
            if (lexer.token == Token.IDENTIFIER && lexer.stringVal().equalsIgnoreCase("TEMPORAY")) {
                lexer.nextToken();
                createTable.setType(wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type.LOCAL_TEMPORARY);
            } else {
                throw new ParserException("syntax error. " + lexer.info());
            }
        }
        ...
        if (lexer.token == Token.LPAREN) {
            ...
            for (; ; ) {
                ...
                if (lexer.identifierEquals(FnvHash.Constants.SUPPLEMENTAL)
                    ...
                    wang.yeting.sql.ast.statement.SQLTableElement element = this.parseCreateTableSupplementalLogingProps();
                    element.setParent(createTable);
                    createTable.getTableElementList().add(element);
                } else if (token == Token.IDENTIFIER //
                        || token == Token.LITERAL_ALIAS) {
                    wang.yeting.sql.ast.statement.SQLColumnDefinition column = this.exprParser.parseColumn(createTable);
                    ...
                    createTable.getTableElementList().add((wang.yeting.sql.ast.statement.SQLTableElement) constraint);
            ...
            if (lexer.identifierEquals(FnvHash.Constants.INHERITS)) {
                ...
                createTable.setInherits(new wang.yeting.sql.ast.statement.SQLExprTableSource(inherits));
                accept(Token.RPAREN);
            }
        }
        ...
        if (lexer.token == Token.AS) {
            ...
            wang.yeting.sql.ast.statement.SQLSelect select = null;
    ...
    protected void parseCreateTableRest(wang.yeting.sql.ast.statement.SQLCreateTableStatement stmt) {
    ...
    protected wang.yeting.sql.ast.statement.SQLTableElement parseCreateTableSupplementalLogingProps() {
        throw new ParserException("TODO " + lexer.info());
    }

    protected wang.yeting.sql.ast.statement.SQLCreateTableStatement newCreateStatement() {
        return new wang.yeting.sql.ast.statement.SQLCreateTableStatement(getDbType());
    }
}