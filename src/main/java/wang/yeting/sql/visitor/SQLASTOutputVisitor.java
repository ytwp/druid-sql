...
import wang.yeting.sql.ast.statement.SQLJoinTableSource.JoinType;
import wang.yeting.sql.ast.statement.SQLMergeStatement.MergeInsertClause;
import wang.yeting.sql.ast.statement.SQLMergeStatement.MergeUpdateClause;
...
import wang.yeting.sql.dialect.odps.ast.OdpsNewExpr;
import wang.yeting.sql.dialect.oracle.ast.OracleSegmentAttributes;
import wang.yeting.sql.dialect.oracle.ast.expr.OracleCursorExpr;
import wang.yeting.sql.dialect.oracle.ast.expr.OracleDatetimeExpr;
import wang.yeting.sql.dialect.oracle.ast.stmt.OracleCreatePackageStatement;
import wang.yeting.sql.dialect.oracle.ast.stmt.OracleForStatement;
import wang.yeting.sql.dialect.oracle.ast.stmt.OracleSelectPivot;
import wang.yeting.sql.dialect.oracle.parser.OracleFunctionDataType;
import wang.yeting.sql.dialect.oracle.parser.OracleProcedureDataType;
...
import static wang.yeting.sql.util.Utils.getBoolean;
...
public class SQLASTOutputVisitor extends SQLASTVisitorAdapter implements ParameterizedVisitor, PrintableVisitor {
    ...
    private static int paramCount(wang.yeting.sql.ast.SQLExpr x) {
        if (x instanceof wang.yeting.sql.ast.SQLName) {
            return 1;
        }

        if (x instanceof wang.yeting.sql.ast.expr.SQLAggregateExpr) {
            wang.yeting.sql.ast.expr.SQLAggregateExpr aggregateExpr = (wang.yeting.sql.ast.expr.SQLAggregateExpr) x;
            List<wang.yeting.sql.ast.SQLExpr> args = aggregateExpr.getArguments();
            int paramCount = 1;
            for (wang.yeting.sql.ast.SQLExpr arg : args) {
        ...
        if (x instanceof wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) {
            List<wang.yeting.sql.ast.SQLExpr> params = ((wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) x).getArguments();
            int paramCount = 1;
            for (wang.yeting.sql.ast.SQLExpr param : params) {
                paramCount += paramCount(param);
            }
            return paramCount;
        }

        if (x instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
            return paramCount(((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) x).getLeft())
                    + paramCount(((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) x).getRight());
        }

        if (x instanceof wang.yeting.sql.ast.expr.SQLCaseExpr) {
            return 10;
        }
    ...
    public void addTableMapping(String srcTable, String destTable) {
        ...
        if (srcTable.indexOf('.') >= 0) {
            wang.yeting.sql.ast.SQLExpr expr = SQLUtils.toSQLExpr(srcTable, dbType);
            if (expr instanceof wang.yeting.sql.ast.expr.SQLPropertyExpr) {
                srcTable = ((wang.yeting.sql.ast.expr.SQLPropertyExpr) expr).simplify().toString();
            }
        } else {
    ...
    protected void printAndAccept(List<? extends wang.yeting.sql.ast.SQLObject> nodes, String seperator) {
    ...
    protected void printAndAccept(List<? extends wang.yeting.sql.ast.SQLExpr> nodes, String seperator, boolean parameterized) {
        for (int i = 0, size = nodes.size(); i < size; ++i) {
            if (i != 0) {
                print0(seperator);
            }
    ...
    protected void printSelectList(List<wang.yeting.sql.ast.statement.SQLSelectItem> selectList) {
        this.indentCount++;
        for (int i = 0, lineItemCount = 0, size = selectList.size()
            ...
            boolean methodOrBinary = (!(selectItemExpr instanceof wang.yeting.sql.ast.SQLName))
                    && (selectItemExpr instanceof wang.yeting.sql.ast.expr.SQLMethodInvokeExpr
                    || selectItemExpr instanceof wang.yeting.sql.ast.expr.SQLAggregateExpr
                    || selectItemExpr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr);
            ...
            if (i != 0) {
                wang.yeting.sql.ast.statement.SQLSelectItem preSelectItem = selectList.get(i - 1);
                if (preSelectItem.getAfterCommentsDirect() != null) {
                    ...
                    if (lineItemCount >= selectListNumberOfLine) {
                        ...
                        || selectItemExpr instanceof wang.yeting.sql.ast.expr.SQLQueryExpr
                        || selectItemExpr instanceof wang.yeting.sql.ast.expr.SQLCaseExpr
                        || (selectItemExpr instanceof wang.yeting.sql.ast.expr.SQLCharExpr && ((wang.yeting.sql.ast.expr.SQLCharExpr) selectItemExpr).getText().length() > 20)) {
                    lineItemCount = 0;
                    println();
                }
    ...
    protected void printlnAndAccept(List<? extends wang.yeting.sql.ast.SQLObject> nodes, String seperator) {
        for (int i = 0, size = nodes.size(); i < size; ++i) {
            if (i != 0) {
                println(seperator);
            }

            ((wang.yeting.sql.ast.SQLObject) nodes.get(i)).accept(this);
        }
    }
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLBetweenExpr x) {
        final wang.yeting.sql.ast.SQLExpr testExpr = x.getTestExpr();
        final wang.yeting.sql.ast.SQLExpr beginExpr = x.getBeginExpr();
        final wang.yeting.sql.ast.SQLExpr endExpr = x.getEndExpr();

        boolean quote = false;
        if (testExpr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
            wang.yeting.sql.ast.expr.SQLBinaryOperator operator = ((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) testExpr).getOperator();
            switch (operator) {
                case BooleanAnd:
                case BooleanOr:
                case BooleanXor:
                case Assignment:
                    ...
                    break;
                default:
                    quote = ((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) testExpr).isParenthesized();
                    break;
            }
        } else if (testExpr instanceof wang.yeting.sql.ast.expr.SQLInListExpr
                || testExpr instanceof wang.yeting.sql.ast.expr.SQLBetweenExpr
                || testExpr instanceof wang.yeting.sql.ast.expr.SQLNotExpr
                || testExpr instanceof wang.yeting.sql.ast.expr.SQLUnaryExpr
                || testExpr instanceof wang.yeting.sql.ast.expr.SQLCaseExpr
                || testExpr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) {
            quote = true;
        }
        ...
        if (beginExpr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
            wang.yeting.sql.ast.expr.SQLBinaryOpExpr binaryOpBegin = (wang.yeting.sql.ast.expr.SQLBinaryOpExpr) beginExpr;
            incrementIndent();
            ...
            decrementIndent();
        } else if (beginExpr instanceof wang.yeting.sql.ast.expr.SQLInListExpr
                || beginExpr instanceof wang.yeting.sql.ast.expr.SQLBetweenExpr
                || beginExpr instanceof wang.yeting.sql.ast.expr.SQLNotExpr
                || beginExpr instanceof wang.yeting.sql.ast.expr.SQLUnaryExpr
                || beginExpr instanceof wang.yeting.sql.ast.expr.SQLCaseExpr
                || beginExpr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) {
            print('(');
            printExpr(beginExpr, parameterized);
            print(')');
        } else {
        ...
        if (endExpr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
            wang.yeting.sql.ast.expr.SQLBinaryOpExpr binaryOpEnd = (wang.yeting.sql.ast.expr.SQLBinaryOpExpr) endExpr;
            incrementIndent();
            ...
            decrementIndent();
        } else if (endExpr instanceof wang.yeting.sql.ast.expr.SQLInListExpr
                || endExpr instanceof wang.yeting.sql.ast.expr.SQLBetweenExpr
                || endExpr instanceof wang.yeting.sql.ast.expr.SQLNotExpr
                || endExpr instanceof wang.yeting.sql.ast.expr.SQLUnaryExpr
                || endExpr instanceof wang.yeting.sql.ast.expr.SQLCaseExpr
                || endExpr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) {
            print('(');
            printExpr(endExpr, parameterized);
            print(')');
        } else {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup x) {
        wang.yeting.sql.ast.SQLObject parent = x.getParent();
        wang.yeting.sql.ast.expr.SQLBinaryOperator operator = x.getOperator();

        boolean isRoot = parent instanceof wang.yeting.sql.ast.statement.SQLSelectQueryBlock || parent instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup;

        List<wang.yeting.sql.ast.SQLExpr> items = x.getItems();
        if (items.size() == 0) {
            print("true");
            return false;
        }
        ...
        if (this.parameterized) {
            wang.yeting.sql.ast.SQLExpr firstLeft = null;
            wang.yeting.sql.ast.expr.SQLBinaryOperator firstOp = null;
            List<Object> parameters = new ArrayList<Object>(items.size());

            List<wang.yeting.sql.ast.expr.SQLBinaryOpExpr> literalItems = null;

            if ((operator != wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanOr || !isEnabled(VisitorFeature.OutputParameterizedQuesUnMergeOr)) &&
                    (operator != wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanAnd || !isEnabled(VisitorFeature.OutputParameterizedQuesUnMergeAnd))) {
                for (int i = 0; i < items.size(); i++) {
                    wang.yeting.sql.ast.SQLExpr item = items.get(i);
                    if (item instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
                        wang.yeting.sql.ast.expr.SQLBinaryOpExpr binaryItem = (wang.yeting.sql.ast.expr.SQLBinaryOpExpr) item;
                        wang.yeting.sql.ast.SQLExpr left = binaryItem.getLeft();
                        wang.yeting.sql.ast.SQLExpr right = binaryItem.getRight();

                        if (right instanceof wang.yeting.sql.ast.expr.SQLLiteralExpr && !(right instanceof wang.yeting.sql.ast.expr.SQLNullExpr)) {
                            if (left instanceof wang.yeting.sql.ast.expr.SQLLiteralExpr) {
                                if (literalItems == null) {
                                    literalItems = new ArrayList<wang.yeting.sql.ast.expr.SQLBinaryOpExpr>();
                                }
                        ...
                        } else if (right instanceof wang.yeting.sql.ast.expr.SQLVariantRefExpr) {
                            // skip
                        } else {
                            firstLeft = null;
                            break;
                        ...
                        if (firstLeft == null) {
                            ...
                            if (firstOp != binaryItem.getOperator() || !wang.yeting.sql.ast.expr.SQLExprUtils.equals(firstLeft, left)) {
                                firstLeft = null;
                                break;
                            }
                        }
            ...
            if (firstLeft != null) {
                if (literalItems != null) {
                    for (wang.yeting.sql.ast.expr.SQLBinaryOpExpr literalItem : literalItems) {
                        visit(literalItem);
                        println();
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (item instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
                ...
                wang.yeting.sql.ast.SQLExpr binaryOpExprRight = binaryOpExpr.getRight();
                wang.yeting.sql.ast.expr.SQLBinaryOperator itemOp = binaryOpExpr.getOperator();
            ...
            } else if (item instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) {
                print('(');
                visit((wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) item);
                print(')');
            } else {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLBinaryOpExpr x) {
        ...
        if (this.parameterized
            ...
            x = wang.yeting.sql.ast.expr.SQLBinaryOpExpr.merge(this, x);
        ...
        if (inputParameters != null
                ...
                && operator == wang.yeting.sql.ast.expr.SQLBinaryOperator.Equality
                && x.getRight() instanceof wang.yeting.sql.ast.expr.SQLVariantRefExpr
        ) {
            wang.yeting.sql.ast.expr.SQLVariantRefExpr right = (wang.yeting.sql.ast.expr.SQLVariantRefExpr) x.getRight();
        ...
        wang.yeting.sql.ast.SQLObject parent = x.getParent();
        boolean isRoot = parent instanceof wang.yeting.sql.ast.statement.SQLSelectQueryBlock;
        boolean relational = operator == wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanAnd
                || operator == wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanOr;
        ...
        List<wang.yeting.sql.ast.SQLExpr> groupList = new ArrayList<wang.yeting.sql.ast.SQLExpr>();
        wang.yeting.sql.ast.SQLExpr left = x.getLeft();
        wang.yeting.sql.ast.SQLExpr right = x.getRight();

        if (inputParameters != null
                && operator != wang.yeting.sql.ast.expr.SQLBinaryOperator.Equality) {
            int varIndex = -1;
            if (right instanceof wang.yeting.sql.ast.expr.SQLVariantRefExpr) {
                varIndex = ((wang.yeting.sql.ast.expr.SQLVariantRefExpr) right).getIndex();
            }
            ...
            if (param instanceof Collection) {
                ...
                if (values.size() > 0) {
                    ...
                    for (Object value : values) {
                        ...
                        print(' ');
                        if (operator == wang.yeting.sql.ast.expr.SQLBinaryOperator.Is) {
                            print('=');
                        } else {
                            printOperator(operator);
                        }
        ...
        if (operator.isRelational()
                && left instanceof wang.yeting.sql.ast.expr.SQLIntegerExpr
                && right instanceof wang.yeting.sql.ast.expr.SQLIntegerExpr) {
            print(((wang.yeting.sql.ast.expr.SQLIntegerExpr) left).getNumber().longValue());
            print(' ');
            printOperator(operator);
            print(' ');
            Number number = ((wang.yeting.sql.ast.expr.SQLIntegerExpr) right).getNumber();
        ...
        for (; ; ) {
            if (left instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr && ((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) left).getOperator() == operator
                    && operator != wang.yeting.sql.ast.expr.SQLBinaryOperator.IsNot && operator != wang.yeting.sql.ast.expr.SQLBinaryOperator.Is) {
                wang.yeting.sql.ast.expr.SQLBinaryOpExpr binaryLeft = (wang.yeting.sql.ast.expr.SQLBinaryOpExpr) left;
        ...
        for (int i = groupList.size() - 1; i >= 0; --i) {
            ...
            if (relational) {
                ...
                if (operator == wang.yeting.sql.ast.expr.SQLBinaryOperator.Modulus
                        && DbType.oracle == dbType
                        && left instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr
                        && right instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr
                        && ((wang.yeting.sql.ast.expr.SQLIdentifierExpr) right).getName().equalsIgnoreCase("NOTFOUND")) {
                    printOpSpace = false;
                }
                if (printOpSpace) {
                    print(' ');
    ...
    protected void printOperator(wang.yeting.sql.ast.expr.SQLBinaryOperator operator) {
        print0(ucase ? operator.name : operator.name_lcase);
    }

    private void visitorBinaryRight(wang.yeting.sql.ast.expr.SQLBinaryOpExpr x) {
        wang.yeting.sql.ast.SQLExpr right = x.getRight();
        wang.yeting.sql.ast.expr.SQLBinaryOperator op = x.getOperator();
        ...
        if (right instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
            wang.yeting.sql.ast.expr.SQLBinaryOpExpr binaryRight = (wang.yeting.sql.ast.expr.SQLBinaryOpExpr) right;
            wang.yeting.sql.ast.expr.SQLBinaryOperator rightOp = binaryRight.getOperator();

            boolean rightRational = rightOp == wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanAnd
                    || rightOp == wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanOr;
        ...
        } else if (right instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) {
            wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup group = (wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) right;
        ...
        } else if (wang.yeting.sql.ast.expr.SQLBinaryOperator.Equality.priority >= op.priority
                && (right instanceof wang.yeting.sql.ast.expr.SQLInListExpr || right instanceof wang.yeting.sql.ast.expr.SQLBetweenExpr || right instanceof wang.yeting.sql.ast.expr.SQLNotExpr)) {
            indentCount++;
            print('(');
    ...
    private void visitBinaryLeft(wang.yeting.sql.ast.SQLExpr left, wang.yeting.sql.ast.expr.SQLBinaryOperator op) {
        if (left instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
            wang.yeting.sql.ast.expr.SQLBinaryOpExpr binaryLeft = (wang.yeting.sql.ast.expr.SQLBinaryOpExpr) left;
            wang.yeting.sql.ast.expr.SQLBinaryOperator leftOp = binaryLeft.getOperator();
            wang.yeting.sql.ast.SQLObject parent = left.getParent();
            boolean leftRational = leftOp == wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanAnd
                    || leftOp == wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanOr;

            final boolean bracket;
            if (leftOp.priority > op.priority) {
                bracket = true;
            } else if (leftOp.priority == op.priority
                    && parent instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr
                    && parent.getParent() instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr
                    && leftOp.priority == ((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) parent.getParent()).getOperator().priority
                    && left == ((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) parent).getRight()) {
                bracket = true;
            } else if ((leftOp == wang.yeting.sql.ast.expr.SQLBinaryOperator.Is || leftOp == wang.yeting.sql.ast.expr.SQLBinaryOperator.IsNot) && !op.isLogical()) {
                bracket = true;
            } else if (binaryLeft.isParenthesized()) {
                if (leftOp != op && ((leftOp.isLogical() && op.isLogical()) || op == wang.yeting.sql.ast.expr.SQLBinaryOperator.Is)) {
                    bracket = true;
                } else {
                    bracket = false;
                }
        ...
        } else if (left instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) {
            wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup group = (wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) left;
        ...
        } else if (left instanceof wang.yeting.sql.ast.expr.SQLInListExpr) {
            wang.yeting.sql.ast.expr.SQLInListExpr inListExpr = (wang.yeting.sql.ast.expr.SQLInListExpr) left;
            boolean quote;
            if (inListExpr.isNot()) {
                quote = op.priority <= wang.yeting.sql.ast.expr.SQLBinaryOperator.Equality.priority;
            } else {
                quote = op.priority < wang.yeting.sql.ast.expr.SQLBinaryOperator.Equality.priority;
            }
            if (quote) {
        ...
        } else if (left instanceof wang.yeting.sql.ast.expr.SQLBetweenExpr) {
            wang.yeting.sql.ast.expr.SQLBetweenExpr betweenExpr = (wang.yeting.sql.ast.expr.SQLBetweenExpr) left;
            boolean quote;
        ...
        } else if (left instanceof wang.yeting.sql.ast.expr.SQLNotExpr) {
            print('(');
            printExpr(left);
            print(')');
        } else if (left instanceof wang.yeting.sql.ast.expr.SQLUnaryExpr) {
            wang.yeting.sql.ast.expr.SQLUnaryExpr unary = (wang.yeting.sql.ast.expr.SQLUnaryExpr) left;
            ...
            switch (unary.getOperator()) {
                ...
                case Plus:
                case Negative:
                    quote = op.priority < wang.yeting.sql.ast.expr.SQLBinaryOperator.Add.priority;
                    break;
                default:
                    break;
            }
    ...
    protected void printTableSource(wang.yeting.sql.ast.statement.SQLTableSource x) {
        Class<?> clazz = x.getClass();
        if (clazz == wang.yeting.sql.ast.statement.SQLJoinTableSource.class) {
            visit((wang.yeting.sql.ast.statement.SQLJoinTableSource) x);
        } else if (clazz == wang.yeting.sql.ast.statement.SQLExprTableSource.class) {
            visit((wang.yeting.sql.ast.statement.SQLExprTableSource) x);
        } else if (clazz == wang.yeting.sql.ast.statement.SQLSubqueryTableSource.class) {
            visit((wang.yeting.sql.ast.statement.SQLSubqueryTableSource) x);
        } else {
            x.accept(this);
        }
    }

    protected void printQuery(wang.yeting.sql.ast.statement.SQLSelectQuery x) {
        Class<?> clazz = x.getClass();
        if (clazz == wang.yeting.sql.ast.statement.SQLSelectQueryBlock.class) {
            visit((wang.yeting.sql.ast.statement.SQLSelectQueryBlock) x);
        } else if (clazz == wang.yeting.sql.ast.statement.SQLUnionQuery.class) {
            visit((wang.yeting.sql.ast.statement.SQLUnionQuery) x);
        } else {
            x.accept(this);
        }
    }

    protected final void printExpr(wang.yeting.sql.ast.SQLExpr x) {
        printExpr(x, parameterized);
    }

    protected final void printExpr(wang.yeting.sql.ast.SQLExpr x, boolean parameterized) {
        Class<?> clazz = x.getClass();
        ...
        } else if (clazz == wang.yeting.sql.ast.expr.SQLAllColumnExpr.class) {
            print('*');
        } else if (clazz == wang.yeting.sql.ast.expr.SQLAggregateExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLAggregateExpr) x);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLBinaryOpExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) x);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLCharExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLCharExpr) x, parameterized);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLNullExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLNullExpr) x);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLIntegerExpr.class) {
            printInteger((wang.yeting.sql.ast.expr.SQLIntegerExpr) x, parameterized);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLNumberExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLNumberExpr) x);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLMethodInvokeExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) x);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLVariantRefExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLVariantRefExpr) x);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup.class) {
            visit((wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) x);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLCaseExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLCaseExpr) x);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLInListExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLInListExpr) x);
        } else if (clazz == wang.yeting.sql.ast.expr.SQLNotExpr.class) {
            visit((wang.yeting.sql.ast.expr.SQLNotExpr) x);
        } else {
            x.accept(this);
        }
    }
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLCaseExpr x) {
        ...
        wang.yeting.sql.ast.SQLExpr valueExpr = x.getValueExpr();
        if (valueExpr != null) {
            printExpr(valueExpr, parameterized);
        }

        List<wang.yeting.sql.ast.expr.SQLCaseExpr.Item> items = x.getItems();
        for (int i = 0, size = items.size(); i < size; ++i) {
            println();
            visit(items.get(i));
        }

        wang.yeting.sql.ast.SQLExpr elExpr = x.getElseExpr();
        if (elExpr != null) {
            println();
            print0(ucase ? "ELSE " : "else ");
            if (elExpr instanceof wang.yeting.sql.ast.expr.SQLCaseExpr) {
                this.indentCount++;
                println();
                visit((wang.yeting.sql.ast.expr.SQLCaseExpr) elExpr);
                this.indentCount--;
            } else {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLCaseExpr.Item x) {
        print0(ucase ? "WHEN " : "when ");
        wang.yeting.sql.ast.SQLExpr conditionExpr = x.getConditionExpr();
        int lines = this.lines;
        this.indentCount++;
        ...
        wang.yeting.sql.ast.SQLExpr valueExpr = x.getValueExpr();
        if (valueExpr instanceof wang.yeting.sql.ast.expr.SQLCaseExpr) {
            this.indentCount++;
            println();
            visit((wang.yeting.sql.ast.expr.SQLCaseExpr) valueExpr);
            this.indentCount--;
        } else {
            this.indentCount++;
            printExpr(valueExpr, parameterized);
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLCaseStatement x) {
        print0(ucase ? "CASE" : "case");
        wang.yeting.sql.ast.SQLExpr valueExpr = x.getValueExpr();
        if (valueExpr != null) {
            print(' ');
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLCaseStatement.Item x) {
        print0(ucase ? "WHEN " : "when ");
        printExpr(x.getConditionExpr(), parameterized);
        print0(ucase ? " THEN " : " then ");

        wang.yeting.sql.ast.SQLStatement stmt = x.getStatement();
        if (stmt != null) {
            stmt.accept(this);
            print(';');
        }
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLCastExpr x) {
        if (x.isTry()) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLCharExpr x, boolean parameterized) {
        if (parameterized) {
            print('?');
            incrementReplaceCunt();
            if (this.parameters != null) {
    ...
    public boolean visit(wang.yeting.sql.ast.SQLDataType x) {
        printDataType(x);

        return false;
    }

    protected void printDataType(wang.yeting.sql.ast.SQLDataType x) {
        boolean parameterized = this.parameterized;
        this.parameterized = false;

        print0(x.getName());
        final List<wang.yeting.sql.ast.SQLExpr> arguments = x.getArguments();
        if (arguments.size() > 0) {
            print('(');
        ...
        if (x instanceof wang.yeting.sql.ast.SQLDataTypeImpl) {
            wang.yeting.sql.ast.SQLExpr indexBy = ((wang.yeting.sql.ast.SQLDataTypeImpl) x).getIndexBy();
            if (indexBy != null) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCharacterDataType x) {
        visit((wang.yeting.sql.ast.SQLDataType) x);

        List<wang.yeting.sql.ast.SQLCommentHint> hints = ((wang.yeting.sql.ast.statement.SQLCharacterDataType) x).hints;
        if (hints != null) {
            print(' ');
            for (wang.yeting.sql.ast.SQLCommentHint hint : hints) {
                hint.accept(this);
            }
        }
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLExistsExpr x) {
        if (x.isNot()) {
        ...
        wang.yeting.sql.ast.SQLCommentHint hint = x.getHint();
        if (hint != null) {
            print(' ');
            hint.accept(this);
        }
    ...
    private boolean printName(wang.yeting.sql.ast.SQLName x, String name) {
    ...
    private boolean printName(wang.yeting.sql.ast.SQLName x, String name, boolean shardingSupport) {

        if (shardingSupport) {
            wang.yeting.sql.ast.SQLObject parent = x.getParent();
            shardingSupport = parent instanceof wang.yeting.sql.ast.statement.SQLExprTableSource || parent instanceof wang.yeting.sql.ast.expr.SQLPropertyExpr;

            if (parent instanceof wang.yeting.sql.ast.expr.SQLPropertyExpr && parent.getParent() instanceof wang.yeting.sql.ast.statement.SQLExprTableSource) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLInListExpr x) {
        final wang.yeting.sql.ast.SQLExpr expr = x.getExpr();

        boolean quote = false;
        if (expr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
            wang.yeting.sql.ast.expr.SQLBinaryOperator operator = ((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) expr).getOperator();
            switch (operator) {
                case BooleanAnd:
                case BooleanOr:
                case BooleanXor:
                case Assignment:
                    ...
                    break;
                default:
                    quote = ((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) expr).isParenthesized();
                    break;
            }
        } else if (expr instanceof wang.yeting.sql.ast.expr.SQLNotExpr
                || expr instanceof wang.yeting.sql.ast.expr.SQLBetweenExpr
                || expr instanceof wang.yeting.sql.ast.expr.SQLInListExpr
                || expr instanceof wang.yeting.sql.ast.expr.SQLUnaryExpr
                || expr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) {
            quote = true;
        }

        if (this.parameterized) {
            List<wang.yeting.sql.ast.SQLExpr> targetList = x.getTargetList();

            boolean allLiteral = true;
            for (wang.yeting.sql.ast.SQLExpr item : targetList) {
                if (!(item instanceof wang.yeting.sql.ast.expr.SQLLiteralExpr || item instanceof wang.yeting.sql.ast.expr.SQLVariantRefExpr)) {
                    if (item instanceof wang.yeting.sql.ast.expr.SQLListExpr) {
                        wang.yeting.sql.ast.expr.SQLListExpr list = (wang.yeting.sql.ast.expr.SQLListExpr) item;
                        for (wang.yeting.sql.ast.SQLExpr listItem : list.getItems()) {
                            if (!(listItem instanceof wang.yeting.sql.ast.expr.SQLLiteralExpr
                                    || listItem instanceof wang.yeting.sql.ast.expr.SQLVariantRefExpr)) {
                                allLiteral = false;
                                break;
                            }
                        }
            ...
            if (allLiteral) {
                ...
                if (targetList.size() == 1 && targetList.get(0) instanceof wang.yeting.sql.ast.expr.SQLVariantRefExpr) {
                    changed = false;
                }
                ...
                if ((!parameterizedQuesUnMergeInList) || (targetList.size() == 1 && !(targetList.get(0) instanceof wang.yeting.sql.ast.expr.SQLListExpr))) {
//                    if (parameters != null) {
//                        print(" (");
...
//                    }
                    ...
                    for (int i = 0; i < targetList.size(); i++) {
                        ...
                        wang.yeting.sql.ast.SQLExpr item = targetList.get(i);
                        if (item instanceof wang.yeting.sql.ast.expr.SQLListExpr) {
                            visit((wang.yeting.sql.ast.expr.SQLListExpr) item);
                            changed = false;
                        } else {
                            print("?");
                        }
                ...
                if (changed) {
                    ...
                    if (this.parameters != null) {
                        if (parameterizedMergeInList) {
                            ...
                            for (wang.yeting.sql.ast.SQLExpr target : x.getTargetList()) {
        ...
        final List<wang.yeting.sql.ast.SQLExpr> list = x.getTargetList();

        boolean printLn = false;
        if (list.size() > 5) {
            printLn = true;
            for (int i = 0, size = list.size(); i < size; ++i) {
                if (!(list.get(i) instanceof wang.yeting.sql.ast.expr.SQLCharExpr)) {
                    printLn = false;
                    break;
                }
            }
        ...
        if (printLn) {
            ...
            for (int i = 0, size = list.size(); i < size; ++i) {
                ...
                wang.yeting.sql.ast.SQLExpr item = list.get(i);
                printExpr(item, parameterized);
            }
            this.indentCount--;
            println();
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLContainsExpr x) {
    ...
    protected void printInteger(wang.yeting.sql.ast.expr.SQLIntegerExpr x, boolean parameterized) {
        ...
        if (number.equals(ONE)) {
            if (DbType.oracle.equals(dbType)) {
                ...
                if (parent instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
                    wang.yeting.sql.ast.expr.SQLBinaryOpExpr binaryOpExpr = (wang.yeting.sql.ast.expr.SQLBinaryOpExpr) parent;
                    wang.yeting.sql.ast.SQLExpr left = binaryOpExpr.getLeft();
                    wang.yeting.sql.ast.expr.SQLBinaryOperator op = binaryOpExpr.getOperator();
                    if (left instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr
                            && op == wang.yeting.sql.ast.expr.SQLBinaryOperator.Equality) {
                        String name = ((wang.yeting.sql.ast.expr.SQLIdentifierExpr) left).getName();
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLMethodInvokeExpr x) {
        ...
        if (parameterized) {
            List<wang.yeting.sql.ast.SQLExpr> arguments = x.getArguments();
    ...
    protected void printMethodParameters(wang.yeting.sql.ast.expr.SQLMethodInvokeExpr x) {
        String function = x.getMethodName();
        List<wang.yeting.sql.ast.SQLExpr> parameters = x.getArguments();
        ...
        for (int i = 0, size = parameters.size(); i < size; ++i) {
            ...
            wang.yeting.sql.ast.SQLExpr param = parameters.get(i);

            if (this.parameterized) {
                if (size == 2 && i == 1 && param instanceof wang.yeting.sql.ast.expr.SQLCharExpr) {
                    if (DbType.oracle == dbType) {
                        if ("TO_CHAR".equalsIgnoreCase(function)
                                || "TO_DATE".equalsIgnoreCase(function)) {
                            printChars(((wang.yeting.sql.ast.expr.SQLCharExpr) param).getText());
                            continue;
                        }
            ...
            if (param instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
                wang.yeting.sql.ast.expr.SQLBinaryOpExpr binaryOpExpr = (wang.yeting.sql.ast.expr.SQLBinaryOpExpr) param;
                wang.yeting.sql.ast.expr.SQLBinaryOperator op = binaryOpExpr.getOperator();
        ...
        if (x.methodNameHashCode64() == FnvHash.Constants.WEIGHT_STRING) {
            wang.yeting.sql.ast.SQLDataType as = (wang.yeting.sql.ast.SQLDataType) x.getAttribute("as");
            if (as != null) {
                print0(ucase ? " AS " : " as ");
                as.accept(this);
            }

            List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem> levels = (List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem>) x.getAttribute("levels");
    ...
    protected void printMethodOwner(wang.yeting.sql.ast.SQLExpr owner) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLAggregateExpr x) {
        ...
        if (x.methodNameHashCode64() == FnvHash.Constants.COUNT) {
            ...
            if (arguments.size() == 1) {
                ...
                if (arg0 instanceof wang.yeting.sql.ast.expr.SQLLiteralExpr) {
                    this.parameterized = false;
                }
            }
        }
        ...
        wang.yeting.sql.ast.expr.SQLAggregateOption option = x.getOption();
        if (option != null) {
        ...
        wang.yeting.sql.ast.SQLKeep keep = x.getKeep();
        if (keep != null) {
            print(' ');
            visit(keep);
        }
        ...
        final wang.yeting.sql.ast.SQLName overRef = x.getOverRef();
        if (overRef != null) {
            print0(ucase ? " OVER " : " over ");
            overRef.accept(this);
        }

        final wang.yeting.sql.ast.SQLExpr filter = x.getFilter();
        if (filter != null) {
    ...
    protected void visitAggreateRest(wang.yeting.sql.ast.expr.SQLAggregateExpr x) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLNotExpr x) {
        ...
        } else if (expr instanceof wang.yeting.sql.ast.expr.SQLInListExpr || expr instanceof wang.yeting.sql.ast.expr.SQLNotExpr
                || expr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) {
            needQuote = true;
        }
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLNullExpr x) {
        wang.yeting.sql.ast.SQLObject parent = x.getParent();
        if (this.parameterized
                && (parent instanceof ValuesClause || parent instanceof wang.yeting.sql.ast.expr.SQLInListExpr ||
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLPropertyExpr x) {
        ...
        if (owner instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
            ownerName = ((wang.yeting.sql.ast.expr.SQLIdentifierExpr) owner).getName();
            if (tableMapping != null) {
        ...
        if (mapTableName != null) {
            ...
            if (owner instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
                wang.yeting.sql.ast.expr.SQLIdentifierExpr ownerIdent = (wang.yeting.sql.ast.expr.SQLIdentifierExpr) owner;
    ...
    protected boolean isTableSourceAlias(wang.yeting.sql.ast.statement.SQLTableSource from, String... tableNames) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLQueryExpr x) {
        wang.yeting.sql.ast.SQLObject parent = x.getParent();
        if (parent instanceof wang.yeting.sql.ast.statement.SQLSelect) {
        ...
        if (parent instanceof ValuesClause) {
            ...
            print(')');
            println();
        } else if ((parent instanceof wang.yeting.sql.ast.SQLStatement
            ...
            this.indentCount--;
        } else if (parent instanceof wang.yeting.sql.ast.statement.SQLOpenStatement) {
            visit(subQuery);
        } else if (parent instanceof wang.yeting.sql.ast.expr.SQLMethodInvokeExpr
                && ((wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) parent).getArguments().size() == 1
                && (((wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) parent).methodNameHashCode64() == FnvHash.Constants.LATERAL
                || ((wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) parent).methodNameHashCode64() == FnvHash.Constants.ARRAY)
        ) {
            this.indentCount++;
            println();
            visit(subQuery);
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSelectGroupByClause x) {
            ...
            for (int i = 0; i < itemSize; ++i) {
                ...
                if (i != 0) {
                    if (groupItemSingleLine) {
                        ...
                        if (item instanceof wang.yeting.sql.ast.expr.SQLGroupingSetExpr) {
                            println();
                        } else {
                            print(", ");
                        }
                ...
                if (item instanceof wang.yeting.sql.ast.expr.SQLIntegerExpr) {
                    printInteger((wang.yeting.sql.ast.expr.SQLIntegerExpr) item, false);
                } else if (item instanceof MySqlOrderingExpr && ((MySqlOrderingExpr) item).getExpr() instanceof wang.yeting.sql.ast.expr.SQLIntegerExpr) {
                    MySqlOrderingExpr orderingExpr = (MySqlOrderingExpr) item;
                    printInteger((wang.yeting.sql.ast.expr.SQLIntegerExpr) orderingExpr.getExpr(), false);
                    print(' ' + orderingExpr.getType().name);
                } else {
                    item.accept(this);
                }

                wang.yeting.sql.ast.SQLCommentHint hint = null;
                if (item instanceof wang.yeting.sql.ast.SQLExprImpl) {
                    hint = ((wang.yeting.sql.ast.SQLExprImpl) item).getHint();
                }
                if (hint != null) {
                    hint.accept(this);
                }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSelect x) {
        wang.yeting.sql.ast.SQLHint headHint = x.getHeadHint();
        if (headHint != null) {
            headHint.accept(this);
            println();
        }

        wang.yeting.sql.ast.statement.SQLWithSubqueryClause withSubQuery = x.getWithSubQuery();
        ...
        final wang.yeting.sql.ast.SQLLimit limit = x.getLimit();
        if (limit != null) {
            println();
            limit.accept(this);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSelectQueryBlock x) {
        ...
        if (wang.yeting.sql.ast.SQLSetQuantifier.ALL == distinctOption) {
            print0(ucase ? "ALL " : "all ");
        } else if (wang.yeting.sql.ast.SQLSetQuantifier.DISTINCT == distinctOption) {
            print0(ucase ? "DISTINCT " : "distinct ");
        } else if (wang.yeting.sql.ast.SQLSetQuantifier.UNIQUE == distinctOption) {
        ...
        if (from != null) {
            ...
            boolean printFrom = from instanceof wang.yeting.sql.ast.statement.SQLLateralViewTableSource
                    && ((wang.yeting.sql.ast.statement.SQLLateralViewTableSource) from).getTableSource() == null;
            if (!printFrom) {
                print0(ucase ? "FROM " : "from ");
            }
        ...
        final List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem> distributeBy = x.getDistributeByDirect();
        ...
        List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem> sortBy = x.getSortByDirect();
        ...
        final List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem> clusterBy = x.getClusterByDirect();
    ...
    protected void printFetchFirst(wang.yeting.sql.ast.statement.SQLSelectQueryBlock x) {
        wang.yeting.sql.ast.SQLLimit limit = x.getLimit();
        if (limit == null) {
            return;
        }

        wang.yeting.sql.ast.SQLExpr offset = limit.getOffset();
        wang.yeting.sql.ast.SQLExpr first = limit.getRowCount();
        ...
        if (DbType.informix == dbType) {
            ...
            wang.yeting.sql.ast.SQLObject parent = x.getParent();
            if (parent instanceof wang.yeting.sql.ast.statement.SQLSelect) {
                wang.yeting.sql.ast.SQLOrderBy orderBy = ((wang.yeting.sql.ast.statement.SQLSelect) parent).getOrderBy();
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSelectItem x) {
        ...
        if (expr instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
            printName0(((wang.yeting.sql.ast.expr.SQLIdentifierExpr) expr).getName());
        } else if (expr instanceof wang.yeting.sql.ast.expr.SQLPropertyExpr) {
            visit((wang.yeting.sql.ast.expr.SQLPropertyExpr) expr);
        } else if (expr != null) {
            printExpr(expr, parameterized);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.SQLOrderBy x) {
        List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem> items = x.getItems();
        ...
        if (items.size() > 0) {
            ...
            for (int i = 0, size = items.size(); i < size; ++i) {
                ...
                wang.yeting.sql.ast.statement.SQLSelectOrderByItem item = items.get(i);
                visit(item);
            }
        }
        return false;
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSelectOrderByItem x) {
        wang.yeting.sql.ast.SQLExpr expr = x.getExpr();

        if (expr instanceof wang.yeting.sql.ast.expr.SQLIntegerExpr) {
            print(((wang.yeting.sql.ast.expr.SQLIntegerExpr) expr).getNumber().longValue());
        } else {
            printExpr(expr, parameterized);
        }

        wang.yeting.sql.ast.SQLOrderingSpecification type = x.getType();
        if (type != null) {
            print(' ');
        ...
        wang.yeting.sql.ast.statement.SQLSelectOrderByItem.NullsOrderType nullsOrderType = x.getNullsOrderType();
    ...
    protected void printTableSourceExpr(wang.yeting.sql.ast.SQLExpr expr) {
        if (exportTables) {
        ...
        if (isEnabled(VisitorFeature.OutputDesensitize)) {
            String ident = null;
            if (expr instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
                ident = ((wang.yeting.sql.ast.expr.SQLIdentifierExpr) expr).getName();
            } else if (expr instanceof wang.yeting.sql.ast.expr.SQLPropertyExpr) {
        ...
        if (tableMapping != null && expr instanceof wang.yeting.sql.ast.SQLName) {
            String tableName;
            if (expr instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
                tableName = ((wang.yeting.sql.ast.expr.SQLIdentifierExpr) expr).normalizedName();
            } else if (expr instanceof wang.yeting.sql.ast.expr.SQLPropertyExpr) {
        ...
        if (expr instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
            ...
            if (owner instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
                wang.yeting.sql.ast.expr.SQLIdentifierExpr identOwner = (wang.yeting.sql.ast.expr.SQLIdentifierExpr) owner;
            ...
            visit((wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) expr);
        } else {
            expr.accept(this);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLExprTableSource x) {
        ...
        final wang.yeting.sql.ast.statement.SQLTableSampling sampling = x.getSampling();
        if (sampling != null) {
            print(' ');
        ...
        String alias = x.getAlias();
        List<wang.yeting.sql.ast.SQLName> columns = x.getColumnsDirect();
        if (alias != null) {
            print(' ');
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSelectStatement stmt) {
        List<wang.yeting.sql.ast.SQLCommentHint> headHints = stmt.getHeadHintsDirect();
        if (headHints != null) {
            for (wang.yeting.sql.ast.SQLCommentHint hint : headHints) {
                visit((wang.yeting.sql.ast.SQLCommentHint) hint);
                println();
            }
        }

        wang.yeting.sql.ast.statement.SQLSelect select = stmt.getSelect();
        if (select != null) {
            this.visit(select);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLVariantRefExpr x) {
        ...
        boolean in;
        if (parent instanceof wang.yeting.sql.ast.expr.SQLInListExpr) {
            in = true;
        } else if (parent instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
            wang.yeting.sql.ast.expr.SQLBinaryOpExpr binaryOpExpr = (wang.yeting.sql.ast.expr.SQLBinaryOpExpr) parent;
            if (binaryOpExpr.getOperator() == wang.yeting.sql.ast.expr.SQLBinaryOperator.Equality) {
                in = true;
            } else {
                in = false;
            }
    ...
    public void printParameter(Object param) {
        ...
        if (param instanceof String) {
            wang.yeting.sql.ast.expr.SQLCharExpr charExpr = new wang.yeting.sql.ast.expr.SQLCharExpr((String) param);
            visit(charExpr);
            return;
        }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLDropTableStatement x) {
        List<wang.yeting.sql.ast.SQLCommentHint> headHints = x.getHeadHintsDirect();
        if (headHints != null) {
            for (wang.yeting.sql.ast.SQLCommentHint hint : headHints) {
                visit((wang.yeting.sql.ast.SQLCommentHint) hint);
                println();
        ...
        List<wang.yeting.sql.ast.SQLCommentHint> hints = x.getHints();
        if (hints != null) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLDropEventStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLDropResourceStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLDropResourceGroupStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLColumnDefinition x) {
        ...
        final wang.yeting.sql.ast.SQLDataType dataType = x.getDataType();
        if (dataType != null) {
            print(' ');
        ...
        final wang.yeting.sql.ast.AutoIncrementType sequenceType = x.getSequenceType();
        ...
        for (wang.yeting.sql.ast.statement.SQLColumnConstraint item : x.getConstraints()) {
            boolean newLine = item instanceof wang.yeting.sql.ast.statement.SQLForeignKeyConstraint //
                    || item instanceof wang.yeting.sql.ast.statement.SQLPrimaryKey //
                    || item instanceof wang.yeting.sql.ast.statement.SQLColumnCheck //
                    || item instanceof wang.yeting.sql.ast.statement.SQLColumnCheck //
                    || item.getName() != null;
            if (newLine) {
                this.indentCount++;
        ...
        wang.yeting.sql.ast.SQLExpr generatedAlawsAs = x.getGeneratedAlawsAs();
        if (generatedAlawsAs != null) {
            print0(ucase ? " GENERATED ALWAYS AS " : " generated always as ");
            printExpr(generatedAlawsAs, parameterized);
        }

        wang.yeting.sql.ast.statement.SQLColumnDefinition.Identity identity = x.getIdentity();
        if (identity != null) {
            if (dbType == DbType.h2) {
                print0(ucase ? " AS " : " as ");
            } else {
        ...
        List<wang.yeting.sql.ast.statement.SQLAssignItem> mappedBy = x.getMappedByDirect();
        ...
        List<wang.yeting.sql.ast.statement.SQLAssignItem> colProperties = x.getColPropertiesDirect();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLColumnDefinition.Identity x) {
    ...
    protected void visitColumnDefault(wang.yeting.sql.ast.statement.SQLColumnDefinition x) {
    ...
    public void printInsertColumns(List<wang.yeting.sql.ast.SQLExpr> columns) {
        ...
        if (size > 0) {
            ...
            for (int i = 0; i < size; ++i) {
                ...
                wang.yeting.sql.ast.SQLExpr column = columns.get(i);
                if (column instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
                    visit((wang.yeting.sql.ast.expr.SQLIdentifierExpr) column);
                } else {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLUpdateStatement x) {
        final List<wang.yeting.sql.ast.SQLCommentHint> headHints = x.getHeadHintsDirect();
        if (headHints != null) {
            for (wang.yeting.sql.ast.SQLCommentHint hint : headHints) {
                hint.accept(this);
                println();
        ...
        for (int i = 0, size = x.getItems().size(); i < size; ++i) {
            ...
            wang.yeting.sql.ast.statement.SQLUpdateSetItem item = (wang.yeting.sql.ast.statement.SQLUpdateSetItem) x.getItems().get(i);
            visit(item);
        }
    ...
    protected void printTableElements(List<wang.yeting.sql.ast.statement.SQLTableElement> tableElementList) {
        ...
        println();
        for (int i = 0; i < size; ++i) {
            wang.yeting.sql.ast.statement.SQLTableElement element = tableElementList.get(i);
            element.accept(this);
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateTableStatement x) {
        ...
        wang.yeting.sql.ast.SQLPartitionBy partitionBy = x.getPartitioning();
        if (partitionBy != null) {
            println();
        ...
        List<wang.yeting.sql.ast.statement.SQLAssignItem> options = x.getTableOptions();
        if (options.size() > 0) {
            println();
        ...
        wang.yeting.sql.ast.SQLName tablespace = x.getTablespace();
        if (tablespace != null) {
            println();
        ...
        wang.yeting.sql.ast.SQLExpr engine = x.getEngine();
        if (engine != null) {
    ...
    protected void printCreateTable(wang.yeting.sql.ast.statement.SQLCreateTableStatement x, boolean printSelect) {
        ...
        final wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type tableType = x.getType();
        if (wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type.GLOBAL_TEMPORARY.equals(tableType)) {
            print0(ucase ? "GLOBAL TEMPORARY " : "global temporary ");
        } else if (wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type.LOCAL_TEMPORARY.equals(tableType)) {
            print0(ucase ? "LOCAL TEMPORARY " : "local temporary ");
        } else if (wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type.SHADOW.equals(tableType)) {
        ...
        wang.yeting.sql.ast.statement.SQLExprTableSource inherits = x.getInherits();
        if (inherits != null) {
        ...
        wang.yeting.sql.ast.SQLExpr storedAs = x.getStoredAs();
        if (storedAs != null) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLUniqueConstraint x) {
        if (x.getName() != null) {
        ...
        print0(ucase ? "UNIQUE (" : "unique (");
        List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem> columns = x.getColumns();
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLNotNullConstraint x) {
        ...
        List<wang.yeting.sql.ast.SQLCommentHint> hints = x.hints;
        if (hints != null) {
            print(' ');
            for (wang.yeting.sql.ast.SQLCommentHint hint : hints) {
                hint.accept(this);
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLNullConstraint x) {
        wang.yeting.sql.ast.SQLName name = x.getName();
        if (name != null) {
            print0(ucase ? "CONSTRAINT " : "constraint ");
            name.accept(this);
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLUnionQuery x) {
        ...
        if (relations.size() > 2) {
            for (int i = 0; i < relations.size(); i++) {
                ...
                wang.yeting.sql.ast.statement.SQLSelectQuery item = relations.get(i);
                item.accept(this);
            }
        ...
        if ((!bracket)
            ...
            if (right instanceof wang.yeting.sql.ast.statement.SQLSelectQueryBlock) {
                wang.yeting.sql.ast.statement.SQLSelectQueryBlock rightQueryBlock = (wang.yeting.sql.ast.statement.SQLSelectQueryBlock) right;
            ...
            if (leftUnion.getRelations().size() > 2) {
                ...
                for (; ; ) {
                    ...
                    if (leftRight instanceof wang.yeting.sql.ast.statement.SQLSelectQueryBlock) {
                        wang.yeting.sql.ast.statement.SQLSelectQueryBlock leftRightQueryBlock = (wang.yeting.sql.ast.statement.SQLSelectQueryBlock) leftRight;
        ...
        if (left != null) {
            for (; ; ) {
                if (left.getClass() == wang.yeting.sql.ast.statement.SQLUnionQuery.class) {
                    wang.yeting.sql.ast.statement.SQLUnionQuery leftUnion = (wang.yeting.sql.ast.statement.SQLUnionQuery) left;
                    if (leftUnion.getRelations().size() > 2) {
                        visit(leftUnion);
                    } else {
                        wang.yeting.sql.ast.statement.SQLSelectQuery leftLeft = leftUnion.getLeft();
                        wang.yeting.sql.ast.statement.SQLSelectQuery leftRigt = leftUnion.getRight();
                        if ((!leftUnion.isParenthesized())
                                && leftUnion.getRight() instanceof wang.yeting.sql.ast.statement.SQLSelectQueryBlock
                                && leftUnion.getLeft() != null
                                && leftUnion.getLimit() == null
                                && leftUnion.getOrderBy() == null) {
                            if (leftLeft.getClass() == wang.yeting.sql.ast.statement.SQLUnionQuery.class) {
                                visit((wang.yeting.sql.ast.statement.SQLUnionQuery) leftLeft);
                            } else {
                                printQuery(leftLeft);
                            }
                            println();
        ...
        if ((!right.isParenthesized())
                && right instanceof wang.yeting.sql.ast.statement.SQLSelectQueryBlock) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLUnaryExpr x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSetStatement x) {
        if (x.getHeadHintsDirect() != null) {
            for (wang.yeting.sql.ast.SQLCommentHint hint : x.getHeadHintsDirect()) {
                hint.accept(this);
            }
        }
        ...
        wang.yeting.sql.ast.statement.SQLSetStatement.Option option = x.getOption();
        if (option != null) {
            print(option.name());
            print(' ');
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAssignItem x) {
        x.getTarget().accept(this);

        wang.yeting.sql.ast.SQLExpr value = x.getValue();
        if (value != null) {
            print0(" = ");
            value.accept(this);
        } else {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLJoinTableSource x) {
        ...
        wang.yeting.sql.ast.statement.SQLTableSource left = x.getLeft();
        ...
        if (left instanceof wang.yeting.sql.ast.statement.SQLJoinTableSource
                && ((wang.yeting.sql.ast.statement.SQLJoinTableSource) left).getJoinType() == JoinType.COMMA
                && joinType != JoinType.COMMA
                && DbType.postgresql != dbType
                && ((wang.yeting.sql.ast.statement.SQLJoinTableSource) left).getAttribute("ads.comma_join") == null) {
            print('(');
        ...
        wang.yeting.sql.ast.statement.SQLTableSource right = x.getRight();
        if (right instanceof wang.yeting.sql.ast.statement.SQLJoinTableSource) {
            print('(');
        ...
        if (condition != null) {
            ...
            if (right instanceof wang.yeting.sql.ast.statement.SQLSubqueryTableSource) {
                newLine = true;
            } else if (condition instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
                wang.yeting.sql.ast.expr.SQLBinaryOperator op = ((wang.yeting.sql.ast.expr.SQLBinaryOpExpr) condition).getOperator();
                if (op == wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanAnd || op == wang.yeting.sql.ast.expr.SQLBinaryOperator.BooleanOr) {
                    newLine = true;
                }
            } else if (condition instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExprGroup) {
                newLine = true;
            }
            if (newLine) {
                println();
        ...
        wang.yeting.sql.ast.statement.SQLJoinTableSource.UDJ udj = x.getUdj();
        if (udj != null) {
            println();
            udj.accept(this);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLJoinTableSource.UDJ x) {
        print0(ucase ? "USING " : "using ");
        ...
        List<wang.yeting.sql.ast.statement.SQLAssignItem> properties = x.getProperties();
    ...
    public boolean visit(ValuesClause x, List<Object> parameters) {
        ...
        final List<wang.yeting.sql.ast.SQLExpr> values = x.getValues();
        ...
        for (int i = 0, size = values.size(); i < size; ++i) {
            ...
            if (value instanceof wang.yeting.sql.ast.SQLExpr) {
                wang.yeting.sql.ast.SQLExpr expr = values.get(i);

                if (this.parameterized &&
                        (expr instanceof wang.yeting.sql.ast.expr.SQLIntegerExpr
                                || expr instanceof wang.yeting.sql.ast.expr.SQLBooleanExpr
                                || expr instanceof wang.yeting.sql.ast.expr.SQLNumberExpr
                                || expr instanceof wang.yeting.sql.ast.expr.SQLCharExpr
                                || expr instanceof wang.yeting.sql.ast.expr.SQLNCharExpr
                                || expr instanceof wang.yeting.sql.ast.expr.SQLTimestampExpr
                                || expr instanceof wang.yeting.sql.ast.expr.SQLDateExpr
                                || expr instanceof wang.yeting.sql.ast.expr.SQLTimeExpr)) {
                    print('?');
                    incrementReplaceCunt();
                    if (parameters != null) {
                    ...
                    continue;
                } else if (this.parameterized && expr instanceof wang.yeting.sql.ast.expr.SQLNullExpr) {
                    print('?');
                    if (parameters != null) {
                        parameters.add(null);
                    }
                    incrementReplaceCunt();
                } else if (expr instanceof wang.yeting.sql.ast.expr.SQLVariantRefExpr) {
                    visit((wang.yeting.sql.ast.expr.SQLVariantRefExpr) expr);
                } else {
                    expr.accept(this);
                }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLSomeExpr x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLAnyExpr x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLAllExpr x) {
        print0(ucase ? "ALL (" : "all (");
        this.indentCount++;
        println();
        x.getSubQuery().accept(this);
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLListExpr x) {
        print('(');
        printAndAccept(x.getItems(), ", ");
        print(')');
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSubqueryTableSource x) {
        ...
        final List<wang.yeting.sql.ast.SQLName> columns = x.getColumns();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLUnnestTableSource x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLDefaultExpr x) {
        print0(ucase ? "DEFAULT" : "default");
        return false;
    }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableReplaceColumn x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableDropColumnItem x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableDeleteByCondition x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLDropIndexStatement x) {
        ...
        wang.yeting.sql.ast.SQLExpr algorithm = x.getAlgorithm();
        if (algorithm != null) {
            print0(ucase ? " ALGORITHM " : " algorithm ");
            algorithm.accept(this);
        }

        wang.yeting.sql.ast.SQLExpr lockOption = x.getLockOption();
        if (lockOption != null) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLDropLogFileGroupStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLDropServerStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSavePointStatement x) {
        ...
        wang.yeting.sql.ast.SQLExpr name = x.getName();
        if (name != null) {
            print(' ');
            name.accept(this);
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLReleaseSavePointStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.SQLCommentHint x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateDatabaseStatement x) {
        ...
        final wang.yeting.sql.ast.SQLExpr comment = x.getComment();
        if (comment != null) {
            print0(ucase ? " COMMENT " : " comment ");
            printExpr(comment, false);
        }

        final wang.yeting.sql.ast.SQLExpr location = x.getLocation();
        if (location != null) {
        ...
        if (x.getOptions().size() > 0) {
            ...
            for (Map.Entry<String, wang.yeting.sql.ast.SQLExpr> option : x.getOptions().entrySet()) {
        ...
        final List<List<wang.yeting.sql.ast.statement.SQLAssignItem>> storedBy = x.getStoredBy();
        if (storedBy.size() != 0) {
            println();
            ...
            for (int i = 0; i < storedBy.size(); i++) {
                List<wang.yeting.sql.ast.statement.SQLAssignItem> items = storedBy.get(i);
                if (i != 0) {
                    print(',');
                    println();
                }
        ...
        final wang.yeting.sql.ast.SQLExpr storedIn = x.getStoredIn();
        if (storedIn != null) {
            println();
            print0(ucase ? "STORED IN " : "stored in ");
            printExpr(storedIn);

            final List<wang.yeting.sql.ast.statement.SQLAssignItem> storedOn = x.getStoredOn();
            if (storedOn.size() > 0) {
                print0(ucase ? " ON (" : " ON (");
        ...
        final wang.yeting.sql.ast.SQLExpr storedAs = x.getStoredAs();
        if (storedAs != null) {
            println();
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateTableGroupStatement x) {
        ...
        wang.yeting.sql.ast.SQLExpr partitionNum = x.getPartitionNum();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableGroupStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterSystemGetConfigStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterSystemSetConfigStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterViewStatement x) {
        ...
        wang.yeting.sql.ast.SQLName definer = x.getDefiner();
        if (definer != null) {
        ...
        List<wang.yeting.sql.ast.statement.SQLTableElement> columns = x.getColumns();
        if (columns.size() > 0) {
            print0(" (");
            this.indentCount++;
            println();
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateViewStatement x) {
        ...
        wang.yeting.sql.ast.SQLName to = x.getTo();
        if (to != null) {
            print0(ucase ? " TO " : " to ");
            to.accept(this);
        }
        ...
        wang.yeting.sql.ast.expr.SQLLiteralExpr comment = x.getComment();
        if (comment != null) {
            println();
            print0(ucase ? "COMMENT " : "comment ");
            comment.accept(this);
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateViewStatement.Column x) {
        x.getExpr().accept(this);

        wang.yeting.sql.ast.expr.SQLCharExpr comment = x.getComment();
        if (comment != null) {
            print0(ucase ? " COMMENT " : " comment ");
            comment.accept(this);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.SQLOver x) {
        ...
        wang.yeting.sql.ast.SQLOrderBy distributeBy = x.getDistributeBy();
        if (distributeBy != null) {
            List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem> items = distributeBy.getItems();
...
//        }
        ...
        final wang.yeting.sql.ast.SQLExpr windowingBetweenBegin = x.getWindowingBetweenBegin();
        final wang.yeting.sql.ast.SQLExpr windowingBetweenEnd = x.getWindowingBetweenEnd();
        final wang.yeting.sql.ast.SQLOver.WindowingBound beginBound = x.getWindowingBetweenBeginBound();
        final wang.yeting.sql.ast.SQLOver.WindowingBound endBound = x.getWindowingBetweenEndBound();
        final wang.yeting.sql.ast.SQLOver.WindowingType windowingType = x.getWindowingType();
    ...
    void printWindowingExpr(wang.yeting.sql.ast.SQLExpr expr) {
        if (expr == null) {
            return;
        }
        print(' ');
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLKeep x) {
        if (x.getDenseRank() == wang.yeting.sql.ast.SQLKeep.DenseRank.FIRST) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLColumnPrimaryKey x) {
        wang.yeting.sql.ast.SQLName name = x.getName();
        if (name != null) {
            print0(ucase ? "CONSTRAINT " : "constraint ");
            name.accept(this);
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLColumnUniqueKey x) {
        wang.yeting.sql.ast.SQLName name = x.getName();
        if (name != null) {
            print0(ucase ? "CONSTRAINT " : "constraint ");
            name.accept(this);
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLColumnCheck x) {
        wang.yeting.sql.ast.SQLName name = x.getName();
        if (name != null) {
            print0(ucase ? "CONSTRAINT " : "constraint ");
            name.accept(this);
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLWithSubqueryClause x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLWithSubqueryClause.Entry x) {
        ...
        this.indentCount++;
        println();
        wang.yeting.sql.ast.statement.SQLSelect query = x.getSubQuery();
        if (query != null) {
            query.accept(this);
        } else {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableModifyClusteredBy x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableAlterColumn x) {
        ...
        wang.yeting.sql.ast.SQLName originColumn = x.getOriginColumn();
        ...
        final wang.yeting.sql.ast.SQLName after = x.getAfter();
        if (after != null) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTablePartitionCount x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableBlockSize x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableEnableKeys x) {
        print0(ucase ? "ENABLE KEYS" : "enable keys");
        return false;
    }

    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableDisableKeys x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableStatement x) {
        ...
        for (int i = 0; i < x.getItems().size(); ++i) {
            wang.yeting.sql.ast.statement.SQLAlterTableItem item = x.getItems().get(i);
            if (i != 0) {
                wang.yeting.sql.ast.statement.SQLAlterTableItem former = x.getItems().get(i - 1);
                if (this.dbType == DbType.hive
                        && former instanceof wang.yeting.sql.ast.statement.SQLAlterTableAddPartition
                        && item instanceof wang.yeting.sql.ast.statement.SQLAlterTableAddPartition) {
                    // ignore comma
                } else {
                    print(',');
                }
        ...
        List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem> clusteredBy = x.getClusteredBy();
        ...
        List<wang.yeting.sql.ast.statement.SQLSelectOrderByItem> sortedBy = x.getSortedBy();
        if (sortedBy.size() > 0) {
            println();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLExprHint x) {
        x.getExpr().accept(this);
        return false;
    }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateIndexStatement x) {
        ...
        List<wang.yeting.sql.ast.SQLName> covering = x.getCovering();
        if (covering.size() > 0) {
        ...
        final wang.yeting.sql.ast.SQLExpr dbPartitionBy = x.getDbPartitionBy();
        if (dbPartitionBy != null) {
            print0(ucase ? " DBPARTITION BY " : " dbpartition by ");
            dbPartitionBy.accept(this);
        }

        final wang.yeting.sql.ast.SQLExpr tablePartitionBy = x.getTablePartitionBy();
        if (tablePartitionBy != null) {
            print0(ucase ? " TBPARTITION BY " : " tbpartition by ");
            tablePartitionBy.accept(this);
        }

        wang.yeting.sql.ast.SQLExpr tablePartitions = x.getTablePartitions();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLPrimaryKeyImpl x) {
        wang.yeting.sql.ast.SQLName name = x.getName();
        if (name != null) {
            print0(ucase ? "CONSTRAINT " : "constraint ");
            name.accept(this);
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableRenameColumn x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLColumnReference x) {
        wang.yeting.sql.ast.SQLName name = x.getName();
        if (name != null) {
            print0(ucase ? "CONSTRAINT " : "constraint ");
            name.accept(this);
        ...
        wang.yeting.sql.ast.statement.SQLForeignKeyImpl.Match match = x.getReferenceMatch();
        if (match != null) {
    ...
    @Override
    public void endVisit(wang.yeting.sql.ast.statement.SQLDropSequenceStatement x) {
    ...
    @Override
    public void endVisit(wang.yeting.sql.ast.statement.SQLDropUserStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLCancelJobStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLExplainAnalyzeStatement x) {
    ...
    protected void printGrantPrivileges(wang.yeting.sql.ast.statement.SQLGrantStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLGrantStatement x) {
        ...
        boolean with = false;
        wang.yeting.sql.ast.SQLExpr maxQueriesPerHour = x.getMaxQueriesPerHour();
        ...
        wang.yeting.sql.ast.SQLExpr maxUpdatesPerHour = x.getMaxUpdatesPerHour();
        ...
        wang.yeting.sql.ast.SQLExpr maxConnectionsPerHour = x.getMaxConnectionsPerHour();
        ...
        wang.yeting.sql.ast.SQLExpr maxUserConnections = x.getMaxUserConnections();
    ...
    protected void printGrantOn(wang.yeting.sql.ast.statement.SQLGrantStatement x) {
        if (x.getResource() != null) {
            print0(ucase ? " ON " : " on ");

            wang.yeting.sql.ast.statement.SQLObjectType resourceType = x.getResourceType();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLIndexOptions x) {
        /*
        ...
        */
        ...
        wang.yeting.sql.ast.SQLExpr keyBlockSize = x.getKeyBlockSize();
        ...
        wang.yeting.sql.ast.SQLExpr comment = x.getComment();
        if (comment != null) {
            print0(ucase ? " COMMENT " : " comment ");
        ...
        final List<wang.yeting.sql.ast.statement.SQLAssignItem> options = x.getOtherOptions();
        if (options.size() > 0) {
            for (wang.yeting.sql.ast.statement.SQLAssignItem option : options) {
                print(' ');
                option.accept(this);
            }
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLIndexDefinition x) {
        /*
        if (x.hasConstraint()) {
            print0(ucase ? "CONSTRAINT " : "constraint ");
            if (x.getSymbol() != null) {
        ...
        */
        ...
        final wang.yeting.sql.ast.SQLExpr tablePartitionBy = x.getTbPartitionBy();
        if (tablePartitionBy != null) {
            print0(ucase ? " TBPARTITION BY " : " tbpartition by ");
            tablePartitionBy.accept(this);
        }

        wang.yeting.sql.ast.SQLExpr tablePartitions = x.getTbPartitions();
        if (tablePartitions != null) {
            print0(ucase ? " TBPARTITIONS " : " tbpartitions ");
            tablePartitions.accept(this);
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableAlterIndex x) {
        print0(ucase ? "ALTER " : "alter ");
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateTriggerStatement x) {
        ...
        if (x.isUpdate()) {
            ...
            List<wang.yeting.sql.ast.SQLName> colums = x.getUpdateOfColumns();
            for (wang.yeting.sql.ast.SQLName colum : colums) {
                print(' ');
                colum.accept(this);
            }
        }
        ...
        wang.yeting.sql.ast.SQLExpr when = x.getWhen();
        if (when != null) {
            println();
            print0(ucase ? "WHEN " : "when ");
            when.accept(this);
    ...
    public void endVisit(wang.yeting.sql.ast.expr.SQLBooleanExpr x) {
    }

    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLUnionQueryTableSource x) {
        print('(');
        this.indentCount++;
        println();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterViewRenameStatement x) {
        ...
        wang.yeting.sql.ast.SQLName changeOwnerTo = x.getChangeOwnerTo();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableAddPartition x) {
        boolean printAdd = true;
        if (x.getParent() instanceof wang.yeting.sql.ast.statement.SQLAlterTableStatement) {
            ...
            if (p > 0 && stmt.getChildren().get(p - 1) instanceof wang.yeting.sql.ast.statement.SQLAlterTableAddPartition) {
                printAdd = false;
            }
        }
        ...
        wang.yeting.sql.ast.SQLExpr location = x.getLocation();
        if (location != null) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableAddExtPartition x) {
        print0(ucase ? "ADD " : "add ");
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableDropExtPartition x) {
        print0(ucase ? "DROP " : "drop ");
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableDropPartition x) {
        ...
        if (x.getParent() instanceof wang.yeting.sql.ast.statement.SQLAlterTableStatement) {
            wang.yeting.sql.ast.statement.SQLAlterTableStatement stmt = (wang.yeting.sql.ast.statement.SQLAlterTableStatement) x.getParent();
            int p = stmt.getChildren().indexOf(x);
            if (p > 0 && stmt.getChildren().get(p - 1) instanceof wang.yeting.sql.ast.statement.SQLAlterTableDropPartition) {
                printDrop = false;
            }
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableRenamePartition x) {
        print0(ucase ? "PARTITION (" : "partition (");
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableSetComment x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLPrivilegeItem x) {
    ...
    @Override
    public void endVisit(wang.yeting.sql.ast.statement.SQLPrivilegeItem x) {

    }

    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableSetLifecycle x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableSetLocation x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableEnableLifecycle x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableDisableLifecycle x) {
        if (x.getPartition().size() != 0) {
            print0(ucase ? "PARTITION (" : "partition (");
            printAndAccept(x.getPartition(), ", ");
            print0(") ");
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTablePartition x) {
        if (x.getPartition().size() != 0) {
            print0(ucase ? "PARTITION (" : "partition (");
            printAndAccept(x.getPartition(), ", ");
            print0(") ");
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTablePartitionSetProperties x) {
        if (x.getPartition().size() != 0) {
            print0(ucase ? "PARTITION (" : "partition (");
            printAndAccept(x.getPartition(), ", ");
            print0(") ");
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableTouch x) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLArrayExpr x) {
        ...
        if (expr instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr
                && ((wang.yeting.sql.ast.expr.SQLIdentifierExpr) expr).nameHashCode64() == FnvHash.Constants.ARRAY
                && printNameQuote
        ) {
            print0(((wang.yeting.sql.ast.expr.SQLIdentifierExpr) expr).getName());
        } else if (expr != null) {
            expr.accept(this);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLOpenStatement x) {
        ...
        List<wang.yeting.sql.ast.SQLName> columns = x.getColumns();
        if (columns.size() > 0) {
            print('(');
            printAndAccept(columns, ", ");
            print(')');
        ...
        wang.yeting.sql.ast.SQLExpr forExpr = x.getFor();
        if (forExpr != null) {
            print0(ucase ? " FOR " : " for ");
            forExpr.accept(this);
        }

        List<wang.yeting.sql.ast.SQLExpr> using = x.getUsing();
        if (using.size() > 0) {
            print0(ucase ? " USING " : " using ");
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLGroupingSetExpr x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLIfStatement x) {
        print0(ucase ? "IF " : "if ");
        x.getCondition().accept(this);
        this.indentCount++;
        println();
        for (int i = 0, size = x.getStatements().size(); i < size; ++i) {
            wang.yeting.sql.ast.SQLStatement item = x.getStatements().get(i);
            item.accept(this);
            if (i != size - 1) {
                println();
            }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLIfStatement.Else x) {
        print0(ucase ? "ELSE" : "else");
        this.indentCount++;
        println();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLIfStatement.ElseIf x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLLoopStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLParameter x) {
        wang.yeting.sql.ast.SQLName name = x.getName();
        if (x.getDataType().getName().equalsIgnoreCase("CURSOR")) {
            ...
            this.indentCount++;
            println();
            wang.yeting.sql.ast.statement.SQLSelect select = ((wang.yeting.sql.ast.expr.SQLQueryExpr) x.getDefaultValue()).getSubQuery();
            select.accept(this);
            this.indentCount--;
            ...
            wang.yeting.sql.ast.SQLDataType dataType = x.getDataType();
            ...
            if (DbType.oracle == dbType
                ...
                name.accept(this);
                if (x.getParamType() == wang.yeting.sql.ast.SQLParameter.ParameterType.IN) {
                    print0(ucase ? " IN " : " in ");
                } else if (x.getParamType() == wang.yeting.sql.ast.SQLParameter.ParameterType.OUT) {
                    print0(ucase ? " OUT " : " out ");
                } else if (x.getParamType() == wang.yeting.sql.ast.SQLParameter.ParameterType.INOUT) {
                ...
                if (x.getParamType() == wang.yeting.sql.ast.SQLParameter.ParameterType.IN) {
                    boolean skip = DbType.mysql == dbType
                            && x.getParent() instanceof wang.yeting.sql.ast.statement.SQLCreateFunctionStatement;
    ...
    protected void printParamDefaultValue(wang.yeting.sql.ast.SQLParameter x) {
    ...
    public boolean visit(wang.yeting.sql.ast.SQLDeclareItem x) {
        ...
        if (dataType instanceof wang.yeting.sql.ast.SQLRecordDataType) {
            print0(ucase ? "TYPE " : "type ");
        }
        ...
        if (x.getType() == wang.yeting.sql.ast.SQLDeclareItem.Type.TABLE) {
            print0(ucase ? " TABLE" : " table");
        ...
        } else if (x.getType() == wang.yeting.sql.ast.SQLDeclareItem.Type.CURSOR) {
            print0(ucase ? " CURSOR" : " cursor");
        } else {
    ...
    public boolean visit(wang.yeting.sql.ast.SQLPartitionValue x) {
        if (x.getOperator() == wang.yeting.sql.ast.SQLPartitionValue.Operator.LessThan //
                && DbType.oracle != getDbType() && x.getItems().size() == 1 //
                && x.getItems().get(0) instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
            wang.yeting.sql.ast.expr.SQLIdentifierExpr ident = (wang.yeting.sql.ast.expr.SQLIdentifierExpr) x.getItems().get(0);
        ...
        } else if (x.getOperator() == wang.yeting.sql.ast.SQLPartitionValue.Operator.In) {
    ...
    public boolean visit(wang.yeting.sql.ast.SQLPartition x) {
        ...
        final wang.yeting.sql.ast.SQLObject parent = x.getParent();
        if (parent != null) {
            final wang.yeting.sql.ast.SQLObject parent2 = parent.getParent();
    ...
    public boolean visit(wang.yeting.sql.ast.SQLPartitionByRange x) {
        wang.yeting.sql.ast.SQLExpr interval = x.getInterval();
        if (x.getColumns().size() == 0
                && (interval instanceof wang.yeting.sql.ast.expr.SQLBetweenExpr || interval instanceof wang.yeting.sql.ast.expr.SQLMethodInvokeExpr)) {
            ...
            for (wang.yeting.sql.ast.SQLExpr column : x.getColumns()) {
                if (!(column instanceof wang.yeting.sql.ast.SQLName)) {
                    columns = false;
                    break;
                }
            }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLPartitionByValue x) {
    ...
    private void printSQLPartitions(List<wang.yeting.sql.ast.SQLPartition> partitions) {
    ...
    protected void printPartitionsCountAndSubPartitions(wang.yeting.sql.ast.SQLPartitionBy x) {
        final wang.yeting.sql.ast.SQLExpr partitionsCount = x.getPartitionsCount();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLSubPartitionByRange x) {
        print0(ucase ? "SUBPARTITION BY RANGE " : "subpartition by range ");

        wang.yeting.sql.ast.SQLExpr subPartitionsCount = x.getSubPartitionsCount();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLSubPartitionByList x) {
        if (x.isLinear()) {
            print0(ucase ? "SUBPARTITION BY LINEAR HASH " : "subpartition by linear hash ");
        } else {
            print0(ucase ? "SUBPARTITION BY HASH " : "subpartition by hash ");
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterDatabaseStatement x) {
        ...
        wang.yeting.sql.ast.statement.SQLAlterDatabaseItem item = x.getItem();
        if (item instanceof MySqlAlterDatabaseSetOption) {
            ...
            wang.yeting.sql.ast.SQLName on = setOption.getOn();
            if (on != null) {
                print0(ucase ? " ON " : " on ");
                on.accept(this);
            }
        ...
        wang.yeting.sql.ast.statement.SQLAlterCharacter character = x.getCharacter();
        if (character != null) {
            print(' ');
    ...
    protected void printPartitions(List<wang.yeting.sql.ast.SQLName> partitions) {
    ...
    public boolean visit(MergeUpdateClause x) {
        ...
        wang.yeting.sql.ast.SQLExpr deleteWhere = x.getDeleteWhere();
        if (deleteWhere != null) {
            this.indentCount++;
            println();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLErrorLoggingClause x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateSequenceStatement x) {
        ...
        if (cache != null) {
            if (cache.booleanValue()) {
                ...
                wang.yeting.sql.ast.SQLExpr cacheValue = x.getCacheValue();
                if (cacheValue != null) {
                    print(' ');
        ...
        final wang.yeting.sql.ast.SQLExpr unitCount = x.getUnitCount();
        if (unitCount != null) {
            print0(ucase ? " UNIT COUNT " : " unit count ");
            printExpr(unitCount);
        }

        final wang.yeting.sql.ast.SQLExpr unitIndex = x.getUnitIndex();
        if (unitIndex != null) {
            print0(ucase ? " INDEX " : " index ");
            printExpr(unitIndex);
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterSequenceStatement x) {
        ...
        if (x.isRestart()) {
            ...
            wang.yeting.sql.ast.SQLExpr restartWith = x.getRestartWith();
            if (restartWith != null) {
                print0(ucase ? " WITH " : " with ");
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLDateExpr x) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLTimeExpr x) {
        wang.yeting.sql.ast.SQLExpr literal = x.getLiteral();
        print0(ucase ? "TIME " : "time ");
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLDateTimeExpr x) {
        wang.yeting.sql.ast.SQLExpr literal = x.getLiteral();
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLRealExpr x) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLDecimalExpr x) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLDoubleExpr x) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLFloatExpr x) {
        Float value = x.getValue();
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLSmallIntExpr x) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLTinyIntExpr x) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLBigIntExpr x) {
    ...
    public boolean visit(wang.yeting.sql.ast.SQLLimit x) {
        print0(ucase ? "LIMIT " : "limit ");
        wang.yeting.sql.ast.SQLExpr offset = x.getOffset();
        if (offset != null) {
            printExpr(offset, parameterized);
            print0(", ");
        }

        wang.yeting.sql.ast.SQLExpr rowCount = x.getRowCount();
        if (rowCount != null) {
            printExpr(rowCount, parameterized);
        }

        final List<wang.yeting.sql.ast.SQLExpr> by = x.getBy();
        if (by != null) {
            print0(ucase ? " BY " : " by ");
    ...
    protected void printHierarchical(wang.yeting.sql.ast.statement.SQLSelectQueryBlock x) {
        wang.yeting.sql.ast.SQLExpr startWith = x.getStartWith(), connectBy = x.getConnectBy();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLWhileStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLDeclareStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLReturnStatement x) {
        print0(ucase ? "RETURN" : "return");
    ...
    public void postVisit(wang.yeting.sql.ast.SQLObject x) {
        if (x instanceof wang.yeting.sql.ast.SQLStatement) {
            wang.yeting.sql.ast.SQLStatement stmt = (wang.yeting.sql.ast.SQLStatement) x;
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLArgument x) {
        wang.yeting.sql.ast.SQLParameter.ParameterType type = x.getType();
        if (type != null) {
            print0(type.name());
            print(' ');
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLCommitStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.expr.SQLFlashbackExpr x) {
        print0(x.getType().name());
        print(' ');
        wang.yeting.sql.ast.SQLExpr expr = x.getExpr();
        if (expr instanceof wang.yeting.sql.ast.expr.SQLBinaryOpExpr) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowCreateMaterializedViewStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterMaterializedViewStatement x) {
        ...
        List<wang.yeting.sql.ast.SQLExpr> partitions = x.getPartitions();
    ...
    @Override
    public void endVisit(wang.yeting.sql.ast.statement.SQLAlterMaterializedViewStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateMaterializedViewStatement x) {
        ...
        if (dbType == DbType.mysql) {
            ...
            if (x.getDistributedByType() != null) {
                ...
                wang.yeting.sql.ast.SQLName distributeByType = x.getDistributedByType();
            ...
            for (wang.yeting.sql.ast.statement.SQLAssignItem option : x.getTableOptions()) {
                String key = ((wang.yeting.sql.ast.expr.SQLIdentifierExpr) option.getTarget()).getName();
        ...
        wang.yeting.sql.ast.SQLPartitionBy partitionBy = x.getPartitionBy();
        if (partitionBy != null) {
            println();
            print0(ucase ? "PARTITION BY " : "partition by ");
            partitionBy.accept(this);
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateUserStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLIntervalExpr x) {
        print0(ucase ? "INTERVAL " : "interval ");
        wang.yeting.sql.ast.SQLExpr value = x.getValue();
        value.accept(this);

        wang.yeting.sql.ast.expr.SQLIntervalUnit unit = x.getUnit();
        if (unit != null) {
            print(' ');
    ...
    public boolean visit(OracleDatetimeExpr x) {
        x.getExpr().accept(this);
        wang.yeting.sql.ast.SQLExpr timeZone = x.getTimeZone();

        if (timeZone instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
            if (((wang.yeting.sql.ast.expr.SQLIdentifierExpr) timeZone).getName().equalsIgnoreCase("LOCAL")) {
    ...
    ///////////// for odps & hive
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLLateralViewTableSource x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowErrorsStatement x) {
        print0(ucase ? "SHOW ERRORS" : "show errors");
        return false;
    }

    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowRecylebinStatement x) {
        print0(ucase ? "SHOW RECYCLEBIN" : "show recyclebin");
        return false;
    }

    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowCatalogsStatement x) {
        print0(ucase ? "SHOW CATALOGS" : "show catalogs");
        final wang.yeting.sql.ast.SQLExpr like = x.getLike();
        if (like != null) {
            print0(ucase ? " LIKE " : " like ");
            printExpr(like);
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowFunctionsStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowSessionStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterCharacter x) {
        if (x.getCharacterSet() != null) {
            print0(ucase ? "CHARACTER SET = " : "character set = ");
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLRecordDataType x) {
        print0(ucase ? "RECORD (" : "record (");
        indentCount++;
        println();
        List<wang.yeting.sql.ast.statement.SQLColumnDefinition> columns = x.getColumns();
        for (int i = 0; i < columns.size(); i++) {
            if (i != 0) {
                println();
            }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLBlockStatement x) {
        if (x.getParameters().size() != 0) {
            this.indentCount++;
            if (x.getParent() instanceof wang.yeting.sql.ast.statement.SQLCreateProcedureStatement) {
                wang.yeting.sql.ast.statement.SQLCreateProcedureStatement procedureStatement = (wang.yeting.sql.ast.statement.SQLCreateProcedureStatement) x.getParent();
            ...
            for (int i = 0, size = x.getParameters().size(); i < size; ++i) {
                ...
                wang.yeting.sql.ast.SQLParameter param = x.getParameters().get(i);
                param.accept(this);
                print(';');
            }
        ...
        for (int i = 0, size = x.getStatementList().size(); i < size; ++i) {
            println();
            wang.yeting.sql.ast.SQLStatement stmt = x.getStatementList().get(i);
            stmt.accept(this);
        }
        this.indentCount--;
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCreateProcedureStatement x) {
        ...
        wang.yeting.sql.ast.SQLName authid = x.getAuthid();
        if (authid != null) {
        ...
        if (wrappedSource != null) {
            ...
            if (block != null && !create) {
                ...
                println();
                if (block instanceof wang.yeting.sql.ast.statement.SQLBlockStatement) {
                    wang.yeting.sql.ast.statement.SQLBlockStatement blockStatement = (wang.yeting.sql.ast.statement.SQLBlockStatement) block;
        ...
        if (block != null) {
            ...
            if (block instanceof wang.yeting.sql.ast.statement.SQLBlockStatement
                    && ((wang.yeting.sql.ast.statement.SQLBlockStatement) block).getStatementList().size() > 0) {
                afterSemi = ((wang.yeting.sql.ast.statement.SQLBlockStatement) block).getStatementList().get(0).isAfterSemi();
            }
        }
    ...
    protected boolean hiveVisit(wang.yeting.sql.ast.statement.SQLExternalRecordFormat x) {
        indentCount++;
        ...
        wang.yeting.sql.ast.SQLExpr terminatedBy = x.getTerminatedBy();
        ...
        wang.yeting.sql.ast.SQLExpr escapedBy = x.getEscapedBy();
        if (escapedBy != null) {
            println();
        ...
        wang.yeting.sql.ast.SQLExpr collectionItemsTerminatedBy = x.getCollectionItemsTerminatedBy();
        ...
        wang.yeting.sql.ast.SQLExpr mapKeysTerminatedBy = x.getMapKeysTerminatedBy();
        ...
        wang.yeting.sql.ast.SQLExpr linesTerminatedBy = x.getLinesTerminatedBy();
        ...
        wang.yeting.sql.ast.SQLExpr nullDefinedAs = x.getNullDefinedAs();
        ...
        wang.yeting.sql.ast.SQLExpr serde = x.getSerde();
        if (serde != null) {
            println();
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLExternalRecordFormat x) {
        wang.yeting.sql.ast.SQLExpr delimitedBy = x.getDelimitedBy();
        if (delimitedBy != null) {
            println();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLArrayDataType x) {
        ...
        if (Boolean.TRUE.equals(x.getAttribute("ads.arrayDataType"))) {
            ...
            wang.yeting.sql.ast.SQLDataType componentType = x.getComponentType();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLMapDataType x) {
        print0(ucase ? "MAP<" : "map<");

        wang.yeting.sql.ast.SQLDataType keyType = x.getKeyType();
        wang.yeting.sql.ast.SQLDataType valueType = x.getValueType();
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLStructDataType x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLRowDataType x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLUnionDataType x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLStructDataType.Field x) {
        wang.yeting.sql.ast.SQLName name = x.getName();
        if (name != null) {
            name.accept(this);
        }
        ...
        if (dataType != null) {
            if (x.getParent() instanceof wang.yeting.sql.ast.SQLRowDataType) {
                if (name != null) {
                    print(' ');
                }
            } else {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableMergePartition x) {
        print0(ucase ? "MERGE " : "merge ");
        if (x.isIfExists()) {
            print0(ucase ? "IF EXISTS " : "if exists ");
        }
    ...
    public boolean visit(wang.yeting.sql.ast.SQLPartitionSpec x) {
        print0(ucase ? "PARTITION (" : "partition (");
        printAndAccept(x.getItems(), ", ");
        print(')');
        return false;
    ...
    public boolean visit(wang.yeting.sql.ast.SQLPartitionSpec.Item x) {
        x.getColumn().accept(this);
        print0(" = ");
        x.getValue().accept(this);
        return false;
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableSubpartitionAvailablePartitionNum x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowProcessListStatement x) {
        print0(ucase ? "SHOW " : "show ");
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableSetOption x) {
        ...
        final wang.yeting.sql.ast.SQLExpr on = x.getOn();
        if (on != null) {
            print0(ucase ? " ON " : " on ");
            on.accept(this);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableRenameIndex x) {
    ...
    @Override
    public void endVisit(wang.yeting.sql.ast.expr.SQLMatchAgainstExpr x) {

    }

    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLMatchAgainstExpr x) {
        print0(ucase ? "MATCH (" : "match (");
        printAndAccept(x.getColumns(), ", ");
        print(')');
    ...
    public boolean visit(MySqlCreateTableStatement x) {
        ...
        for (wang.yeting.sql.ast.SQLCommentHint hint : x.getHints()) {
            hint.accept(this);
            print(' ');
        }
        ...
        if (wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type.GLOBAL_TEMPORARY.equals(x.getType())) {
            print0(ucase ? "TEMPORARY TABLE " : "temporary table ");
        } else if (wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type.SHADOW.equals(x.getType())) {
        ...
        List<wang.yeting.sql.ast.statement.SQLAssignItem> tableOptions = x.getTableOptions();
        if (Boolean.TRUE.equals(x.getAttribute("ads.options"))) {
            ...
            for (wang.yeting.sql.ast.statement.SQLAssignItem option : tableOptions) {
                String key = ((wang.yeting.sql.ast.expr.SQLIdentifierExpr) option.getTarget()).getName();
        ...
        if (x.getDistributeByType() != null) {
            ...
            wang.yeting.sql.ast.SQLName distributeByType = x.getDistributeByType();
        ...
        wang.yeting.sql.ast.SQLExpr dbPartitionBy = x.getDbPartitionBy();
        if (dbPartitionBy != null) {
            println();
            print0(ucase ? "DBPARTITION BY " : "dbpartition by ");
            dbPartitionBy.accept(this);
        ...
        wang.yeting.sql.ast.SQLExpr dbpartitions = x.getDbpartitions();
        if (dbpartitions != null) {
            print0(ucase ? " DBPARTITIONS " : " dbpartitions ");
            dbpartitions.accept(this);
        }

        wang.yeting.sql.ast.SQLExpr tbPartitionsBy = x.getTablePartitionBy();
        ...
        if (x.getWith().size() > 0) {
            ...
            print0(ucase ? " WITH (" : " with (");
            int i = 0;
            for (Map.Entry<String, wang.yeting.sql.ast.SQLName> option : x.getWith().entrySet()) {
                if (i != 0) {
                    print0(", ");
                }
                print0(option.getKey());
        ...
        for (wang.yeting.sql.ast.SQLCommentHint hint : x.getOptionHints()) {
            print(' ');
            hint.accept(this);
        }
        return false;
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLValuesExpr x) {
        print0(ucase ? "VALUES (" : "values (");
        printAndAccept(x.getValues(), ", ");
        return false;
    }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLExtractExpr x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLJSONExpr x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLAnnIndex x) {
        print0(ucase ? "ANNINDX (type = '" : "annindx (type = '");

        final int indexType = x.getIndexType();
        if ((indexType & wang.yeting.sql.ast.SQLAnnIndex.IndexType.Flat.mask) != 0) {
            print0(ucase ? "FLAT" : "flat");
        }

        if ((indexType & wang.yeting.sql.ast.SQLAnnIndex.IndexType.FastIndex.mask) != 0) {
            if (indexType != wang.yeting.sql.ast.SQLAnnIndex.IndexType.FastIndex.mask) {
                print(',');
            }
        ...
        if (rtIndexType != 0) {
            ...
            if ((rtIndexType & wang.yeting.sql.ast.SQLAnnIndex.IndexType.Flat.mask) != 0) {
                print0(ucase ? "FLAT" : "flat");
            }

            if ((rtIndexType & wang.yeting.sql.ast.SQLAnnIndex.IndexType.FastIndex.mask) != 0) {
                if (rtIndexType != wang.yeting.sql.ast.SQLAnnIndex.IndexType.FastIndex.mask) {
                    print(',');
                }
                print0(ucase ? "FLAT_INDEX " : "fast_index ");
            }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableRecoverPartitions x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterIndexStatement.Rebuild x) {
        print0(ucase ? "REBUILD" : "rebuild");
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLPartitionRef x) {
        print0(ucase ? "PARTITION (" : "partition (");
        for (int i = 0; i < x.getItems().size(); i++) {
            if (i != 0) {
                print(", ");
            }
            wang.yeting.sql.ast.statement.SQLPartitionRef.Item item = x.getItems().get(i);
            visit(item);
        }
        print(')');
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLPartitionRef.Item x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLExportTableStatement x) {
        ...
        final wang.yeting.sql.ast.SQLExpr to = x.getTo();
        if (to != null) {
            print0(ucase ? " TO " : " to ");
            to.accept(this);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLImportTableStatement x) {
        ...
        final wang.yeting.sql.ast.SQLExpr from = x.getFrom();
        if (from != null) {
            print0(ucase ? " FROM " : " from ");
            from.accept(this);
        }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLTableSampling x) {
        ...
        final wang.yeting.sql.ast.SQLExpr bucket = x.getBucket();
        if (bucket != null) {
            print0(ucase ? "BUCKET " : "bucket ");
            bucket.accept(this);
        }

        final wang.yeting.sql.ast.SQLExpr outOf = x.getOutOf();
        if (outOf != null) {
        ...
        final wang.yeting.sql.ast.SQLExpr percent = x.getPercent();
        if (percent != null) {
            percent.accept(this);
            print0(ucase ? " PERCENT" : " percent");
        }

        final wang.yeting.sql.ast.SQLExpr rows = x.getRows();
        if (rows != null) {
            rows.accept(this);
        ...
        final wang.yeting.sql.ast.SQLExpr size = x.getByteLength();
        if (size != null) {
            size.accept(this);
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.expr.SQLSizeExpr x) {
        x.getValue().accept(this);
        print0(x.getUnit().name());
        return false;
    }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableUnarchivePartition x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowQueryTaskStatement x) {
        if (x.isFull()) {
        ...
        final wang.yeting.sql.ast.SQLOrderBy orderBy = x.getOrderBy();
        if (orderBy != null) {
            println();
            visit(orderBy);
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowOutlinesStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLPurgeTableStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLPurgeRecyclebinStatement x) {
        print0(ucase ? "PURGE RECYCLEBIN" : "purge recyclebin");
        return false;
    }

    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLPurgeLogsStatement x) {
        print0(ucase ? "PURGE" : "purge");
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLAlterTableAddSupplemental x) {
        print0(ucase ? "ADD " : "add ");
        x.getElement().accept(this);
        return false;
    }

    public boolean visit(wang.yeting.sql.ast.expr.SQLDbLinkExpr x) {
        wang.yeting.sql.ast.SQLExpr expr = x.getExpr();
        if (expr instanceof wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) {
            wang.yeting.sql.ast.expr.SQLMethodInvokeExpr methodInvokeExpr = (wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) x.getExpr();
            wang.yeting.sql.ast.SQLExpr owner = methodInvokeExpr.getOwner();
            if (owner != null) {
                printMethodOwner(owner);
            }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowStatisticStmt x) {
        if (x.isFull()) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowStatisticListStmt x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowPackagesStatement x) {
        print0(ucase ? "SHOW PACKAGES" : "show packages");
        return false;
    }

    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowGrantsStatement x) {
        print0(ucase ? "SHOW GRANTS" : "show grants");
        wang.yeting.sql.ast.SQLExpr user = x.getUser();
        if (user != null) {
            print0(ucase ? " FOR " : " for ");
            user.accept(this);
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.SQLCurrentTimeExpr x) {
        final wang.yeting.sql.ast.SQLCurrentTimeExpr.Type type = x.getType();
        print(ucase ? type.name : type.name_lower);
        return false;
    }

    @Override
    public boolean visit(wang.yeting.sql.ast.SQLCurrentUserExpr x) {
        print(ucase ? "CURRENT_USER" : "current_user");
        return false;
    }

    @Override
    public boolean visit(wang.yeting.sql.ast.SQLAdhocTableSource x) {
        final wang.yeting.sql.ast.statement.SQLCreateTableStatement definition = x.getDefinition();
    ...
    protected void printCreateTable(HiveCreateTableStatement x, boolean printSelect) {
        ...
        if (parent instanceof wang.yeting.sql.ast.SQLAdhocTableSource) {
        ...
        final wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type tableType = x.getType();
        if (wang.yeting.sql.ast.statement.SQLCreateTableStatement.Type.TEMPORARY.equals(tableType)) {
        ...
        List<wang.yeting.sql.ast.statement.SQLAssignItem> mappedBy = x.getMappedBy();
        if (mappedBy != null && mappedBy.size() > 0) {
            println();
            print0(ucase ? "MAPPED BY (" : "mapped by (");
            printAndAccept(mappedBy, ", ");
        ...
        if (partitionSize > 0) {
            ...
            for (int i = 0; i < partitionSize; ++i) {
                wang.yeting.sql.ast.statement.SQLColumnDefinition column = x.getPartitionColumns().get(i);
                column.accept(this);
        ...
        List<wang.yeting.sql.ast.SQLExpr> skewedBy = x.getSkewedBy();
        if (skewedBy.size() > 0) {
            println();
            ...
            List<wang.yeting.sql.ast.SQLExpr> skewedByOn = x.getSkewedByOn();
        ...
        wang.yeting.sql.ast.statement.SQLExternalRecordFormat format = x.getRowFormat();
        if (format != null) {
            println();
        ...
        Map<String, wang.yeting.sql.ast.SQLObject> serdeProperties = x.getSerdeProperties();
        ...
        if (storedAs != null) {
            println();
            print0(ucase ? "STORED AS" : "stored as");
            if (storedAs instanceof wang.yeting.sql.ast.expr.SQLIdentifierExpr) {
                print(' ');
                printExpr(storedAs, parameterized);
            } else {
                incrementIndent();
        ...
        wang.yeting.sql.ast.SQLExpr metaLifeCycle = x.getMetaLifeCycle();
    ...
    protected void printSerdeProperties(Map<String, wang.yeting.sql.ast.SQLObject> serdeProperties) {
        ...
        println();
        int i = 0;
        for (Map.Entry<String, wang.yeting.sql.ast.SQLObject> option : serdeProperties.entrySet()) {
            if (i != 0) {
                print(",");
                println();
            }
    ...
    protected void printTblProperties(HiveCreateTableStatement x) {
        List<wang.yeting.sql.ast.statement.SQLAssignItem> tblProperties = x.getTblProperties();
        if (tblProperties.size() > 0) {
            ...
            println();
            int i = 0;
            for (wang.yeting.sql.ast.statement.SQLAssignItem property : tblProperties) {
                if (i != 0) {
                    print(",");
                    println();
                }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLWhoamiStatement x) {
        print0(ucase ? "WHO AM I" : "who am i");
        return false;
    }

    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLForStatement x) {
        print0(ucase ? "FOR " : "for ");
        ...
        for (int i = 0, size = x.getStatements().size(); i < size; ++i) {
            wang.yeting.sql.ast.SQLStatement stmt = x.getStatements().get(i);
            stmt.accept(this);
            if (i != size - 1) {
                println();
            }
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLCopyFromStatement x) {
        ...
        wang.yeting.sql.ast.SQLExpr accessKeyId = x.getAccessKeyId();
        wang.yeting.sql.ast.SQLExpr accessKeySecret = x.getAccessKeySecret();
        ...
        List<wang.yeting.sql.ast.statement.SQLAssignItem> options = x.getOptions();
        if (options.size() > 0) {
            print0(ucase ? " WITH (" : " with (");
            printAndAccept(options, ", ");
            print(')');
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLShowUsersStatement x) {
    ...
    public boolean visit(wang.yeting.sql.ast.statement.SQLSyncMetaStatement x) {
        ...
        wang.yeting.sql.ast.SQLName from = x.getFrom();
        if (from != null) {
            print0(ucase ? " FROM " : " from ");
            from.accept(this);
        }

        wang.yeting.sql.ast.SQLExpr like = x.getLike();
        if (like != null) {
            print0(ucase ? " LIKE " : " like ");
            like.accept(this);
        }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLTableLike x) {
        print0(ucase ? "LIKE " : "like ");
        x.getTable().accept(this);
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLValuesQuery x) {
        print0(ucase ? "VALUES " : "values ");
        printAndAccept(x.getValues(), ", ");
        return false;
    }
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLBuildTableStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLExportDatabaseStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLImportDatabaseStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLRenameUserStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLSubmitJobStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLRestoreStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLArchiveTableStatement x) {
    ...
    @Override
    public boolean visit(wang.yeting.sql.ast.statement.SQLBackupStatement x) {
    ...
    public void visitStatementList(List<wang.yeting.sql.ast.SQLStatement> statementList) {
        ...
        for (int i = 0, size = statementList.size(); i < size; i++) {
            wang.yeting.sql.ast.SQLStatement stmt = statementList.get(i);

            if (i > 0) {
                wang.yeting.sql.ast.SQLStatement preStmt = statementList.get(i - 1);
                ...
                if (!(stmt instanceof wang.yeting.sql.ast.statement.SQLSetStatement)) {
                    println();
                }
            }
            {
    ...
    public boolean visit(SQLCreateResourceGroupStatement x) {
        ...
        for (Map.Entry<String, wang.yeting.sql.ast.SQLExpr> entry : x.getProperties().entrySet()) {
            print(' ');
            print(entry.getKey());
            print(" = ");

            wang.yeting.sql.ast.SQLExpr value = entry.getValue();
            if (value instanceof wang.yeting.sql.ast.expr.SQLListExpr) {
                printAndAccept(((wang.yeting.sql.ast.expr.SQLListExpr) value).getItems(), ",");
            } else {
                value.accept(this);
            }
        }
    ...
    public boolean visit(OdpsNewExpr x) {
        ...
        return super.visit((wang.yeting.sql.ast.expr.SQLMethodInvokeExpr) x);
    }

    public boolean visit(wang.yeting.sql.ast.statement.SQLPurgeTemporaryOutputStatement x) {
